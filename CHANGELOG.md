...
## [4.111.0] — 2026-09-06 (provisional -- number re-derived at commit-prep)

**TF-54b — counterfactual territorial "threat prevented" + a reusable `expected_passing` component (PR-S182, ADR-089).** Implements TF-54 v1's (4.108.0) reserved `method="counterfactual"` door: a defender's territory "prevented" value as a proper **expected-minus-realized** (GSAA-analog, the TF-59/ADR-085 idiom) quantity instead of v1's death-location proxy, which under-counts a pass aimed into the territory but intercepted before its recorded SPADL `end`. **Additive -- `method="completed_failed"` stays the DEFAULT and is byte-identical; no VAEP/tracking retrain, no re-materialize.**

- **`silly_kicks.territory.compute_territorial_dominance(..., method="counterfactual", completion_model=)`** (Option D, spec section 5): for every opponent pass aimed into a defender's trimmed hull, `expected_threat = P_complete(target) * xT(target)` and `realized_threat = xT(target) if completed else 0`; the headline `territory_xt_prevented_above_expectation = expected - realized` (`Σ (P_complete - outcome) * xT(target)`). A **completed** pass is valued at its observed end (no integration); a **failed** pass's target is unobserved (its SPADL `end` is the death/recovery location, not the intended target) and is modeled as a distribution `q` over the death-direction cone's transition mass, each hypothesized target weighted by `P_complete` and `xT`. Five new counterfactual-only columns (`territory_expected_threat_faced`, `territory_xt_prevented_above_expectation`, `territory_passes_aimed_into_hull`, `territory_mean_completion_faced`, `territory_target_source`) appear ONLY under `method="counterfactual"` (`columns_for_method`, SPEC-04) -- `completed_failed`'s shape and values are untouched. Membership: a completed pass counts iff its end is point-in-hull (same v1 rule, so `territory_xt_conceded` stays formula-identical); a failed pass counts iff its death-direction cone intersects the hull (the re-opened "aimed-in-died-short" case v1 missed). An unresolvable target (cone misses the hull, or transition support below `CounterfactualParams.min_transition_support`) is dropped-and-counted (`territory_target_source="unresolved"`, `TerritoryReport.n_target_modeled`/`n_target_unresolved`), never a fabricated 0 (ADR-042).
- **New reusable `silly_kicks.expected_passing.PassCompletionModel`** -- event-only `P(complete | origin -> target geometry)`: a standardized logistic regression (pure-numpy `sigmoid` serve; sklearn imported only inside `fit`), pickle-free JSON + `SHA256SUMS`, a feature contract + chirality probe, fail-closed `load()` (ADR-011/016/040/044/050 idiom, mirroring `GkCompletionModel`/`GkRetentionModel` but in a new EVENT-ONLY home -- imports `spadl`/`id_compat`/numpy only, never `tracking`, AST import-allowlist gate). Ten event-only features (distance, angle-to-goal, forward/lateral, origin/target x/y, origin/target pitch-third). Trained on observed pass geometry (completed at the real end = 1, failed at the death location = 0); `PassCompletionModel.bundled()` loads the public-corpus-trained default weights (bundled in Commit 2, `training_commit`-stamped). In-repo (not HF-published), the same class as `GkCompletionModel`; `scripts/train_pass_completion.py` generates a `MODEL_CARD.md` from the metrics next to the weights, so the bundle carries a card (ADR-088).
- **New public `xthreat` seam `destination_profiles(model, origin_x, origin_y) -> DestinationProfile`** (SPEC-03): exposes the fitted xT's per-origin destination distribution (`zone_centres`, `zone_values`, raw `probabilities` transition rows) in PHYSICAL SPADL coordinates, with the flat-index + y-inversion convention (ADR-041) owned INSIDE `xthreat` -- `territory` never touches the raw `.transition_matrix`/`.xT`. Family-agnostic: `T` is consumed only as a destination *distribution* (renormalized by the caller over its selected zone subset), valid for both `singh_counts` and `kde_smoothed`. Corrects the `singh_transition_matrix` "row-stochastic" docstring in the same change (SPEC-05).
- **New `expected_passing` C4 container** (dot re-render); the 33 action-coupled `add_*` aggregator count is UNCHANGED (neither the counterfactual method nor the completion model is an `add_*`).
- **Validation is IN-CYCLE, owner-run, reported-not-gated** (spec section 7): a pre-registered battery -- completion-model held-out AUC/ECE/Brier-skill; target-recovery via a synthetic-interception substrate (perturbing a completed pass's true end in both distance AND angle, SPEC-02/SPEC-09) versus the naive "death=intercept" and origin-zone-centroid baselines; the composed mechanism + a locked elite-defender ("Van Dijk") prior as the PRIMARY gate; reliability/discriminant/non-degeneracy/outcome-lens as reported SECONDARY -- via the new owner-run drivers `scripts/train_pass_completion.py` and `scripts/validate_territory_counterfactual.py` (`for_each` shards, ADR-052; `require_clean_tree` provenance, ADR-037; ADR-056 input contract). **Findings: pending owner-run corpus validation** -- this entry is filled in from `docs/research/territory_counterfactual_construct_validity/findings.md` once the owner runs the pre-registered battery on the full WC2022 corpus (Commit 2). Promotion of `counterfactual` to the default method is a SEPARATE follow-on ADR-009 apply decision after the owner reads the report -- NOT this cycle.
- **Per-provider params ship EMPTY** (ADR-009): `CounterfactualParams.for_provider` resolves to the base config until a separate gated apply-PR.
