...
A new event-free `silly_kicks/positioning/` package: a pure simulated-annealing solver `optimise_positions(frame, *, movable, objective, constraints, seed)` behind composable `Objective`/`Constraint`/`Optimizer` protocols, plus a measured frame/team `compute_positioning_gap(frames, *, xt, params)` whose one glossaried column = `threat(actual shape) − threat(best REACHABLE shape) ≥ 0` (gap ≥ 0 by path identity; `xt` injected; deterministic seed from `(game_id, period_id, frame_id)`). `compute_*`, not `add_*` — the action-coupled aggregator count stays 33; +1 C4 container; glossary +3 (456→459); `metric_contracts` registered; NOTICE attribution; **NO bundled weights** (pure solver). `ThreatObjective` computes the pitch-control surface DIRECTLY (never `PitchControlCache`, ADR-043); `pressure_on_target` reuses `pressure_on_actor` (untouched). Additive — no VAEP/tracking retrain, no re-materialize.

- **§8 validity instrument (owner-ratified 2026-09-22, dose retired):** a commit-1 spike MEASURED `positioning_gap` non-monotonic under any dose (it re-anchors to each defender's reachable set) and orthogonal to concurrent shape-badness, so the dose/responsiveness instrument is RETIRED for **optimizer-stability + discrimination** (instrument) and a **predictive** construct gate `corr(gap_t, conceded_threat_{t+Δ})` positive-and-significant. `SAParams.init_sigma_m` 5.0 → 2.0 (seed-invariant optimum, matched to the ~2 m reachable radius).
...
- **Two-commit clean-tree provenance:** commit-1 shipped all code + protocols + solver + compute + probes + docs + C4 + glossary + `metric_contracts` + NOTICE + fixture tests (clean commit-1 SHA `26156e1`). **Commit-2 (this):** the owner-run predictive construct-validity + optimizer-stability battery RAN over the FULL 980-match owner-tier corpus (`docs/research/tf56_positioning/metrics.json`, `run_commit` = `26156e1`) → **GO**: predictive(10 s) r=0.0428, p=9e-185, n=458,175 (positive AND significant — a small effect, but the §8 gate is significance-based by design, since the gap is orthogonal to concurrent threat), discriminating (1932 units, gap_median 0.039), non-degenerate. So the glossaried `positioning_gap` column **SHIPS**. (A NEGATIVE battery would have demoted it in-release — the territorial_defense / TF-60-arms precedent.) Decision: ADR-104.
...
Systemic fix, across ALL 30 `scripts/` corpus drivers, of the defect class the ADR-052 seam left open: every driver still STREAMED `load_matches`, which downloads+parses a match INSIDE the generator before yielding it — so a resumed run re-paid the whole corpus to skip a set of shard writes (resume that costs what it saves), and a match that failed to load raised OUT of the generator, past `for_each`'s per-item `try`, killing the pass. The library gains one small, additive count-extraction API (`ExpectedThreat.zone_counts` + `XtZoneCounts`, the per-match input to the D15 count pass); the rest is scripts-side (`_xt_corpus`, `_events_admission`, `_item_outcome` seams + the `_sb_open_data.fetch_open_360_raw` helper). **`fit()`/`rate()` byte-identical (SK-xT-1 oracle); no VAEP/tracking retrain, no re-materialize, C4-free** (no new subpackage/aggregator/model). Two-commit clean-tree provenance: this commit ships the code; the SkillCorner S1 event-validity verdict artifact (`docs/research/skillcorner_s1_event_validity/verdicts.json`) is owner-run on the DGX and lands in the release's 2nd commit.

### Added — `ExpectedThreat.zone_counts` + `XtZoneCounts` (the count-pass per-match extractor)

`ExpectedThreat.zone_counts(actions) -> XtZoneCounts` computes one match's sparse per-zone shot/goal/move/transition-start counts + transition matrix (the additive input the D15 count pass shards and reduces via `fit_from_counts`, ADR-102). `XtZoneCounts` is a frozen public value object (`zeros`/`__add__`/`as_fit_kwargs`), exported from `silly_kicks.xthreat`. Single-sourced with `fit`: the count-extraction cores in `_grid.py`/`_transitions.py` are shared, so `fit(actions)` is byte-identical (SK-xT-1 frozen-oracle parity holds). Additive — no `NOTICE`/glossary/C4 change, no retrain.

- **`for_each(items, *, load=...)` (D14).** With `load` given, items are cheap REFERENCES: `key(ref)` and the resume/exclusion-marker checks run BEFORE `item = load(ref)`, and `load`+`work` share ONE `try` — so a finished/excluded item is never loaded (resume-before-load), and an unloadable one is a recorded FAILURE, not an exception escaping the generator. Each loader family split into a cheap `Sized` ref lister + a per-ref loader: `_loader_pining.{list_match_refs, load_match, MatchRef, MatchExcluded}`, `_sb_open_data.{list_open_data_refs, load_open_data_match, OpenDataRef}`, `validate_gk_decision.{list_gi_refs, load_gi_match}`. The `pining_source` / `open_data_source` factories return `(refs, load)` in one call (owner-ratified reuse). The streaming `load_matches` / `load_statsbomb_matches` / `load_open_data_matches` wrappers stay thin, byte-identical shims for e2e + ad-hoc callers.
- **Deterministic exclusions are MARKERS (D13).** `scripts/_item_outcome.ItemExcluded` (subclassed by `MatchExcluded`) raised from `load`/`work` writes a `.excluded.json` marker beside the shards, counted in `CorpusPassResult.excluded`, replayed on resume, resetting the consecutive-failure run — distinct from an empty shard ("ran, produced nothing") and a failure. Combiners read `CorpusPassResult.shard_keys` (keys minus failures minus exclusions), never `keys`.
- **xT fits are resumable count passes (D15, ADR-102 amendment).** `scripts/_xt_corpus.{xt_count_pass, fit_xt_from_count_pass}` write one sparse `XtZoneCounts` shard per match (events-only) and reduce via `ExpectedThreat.fit_from_counts` — byte-identical to a pooled `fit(actions)`, refuse-on-failure unless `--allow-failed`. Replaces the whole-corpus-in-memory fit in `build_tf60_layer3_arm_values`, `measure_cover_shadow_argmax_agreement`, `_xtgk_comparability`, `calibrate_xt_bandwidth`, `calibrate_tracking_defaults` (the OOM class is gone).
- **Events-only admission is policy at the edge.** `load_match(events_only=True)` is a PURE loader (runs no S1 geometry gate). Admitting an S1-excluded SkillCorner match into an events-only fit lives ONLY in `scripts/_events_admission.events_only_loader`, against the committed, provenance-stamped verdict artifact; its digest joins `token_inputs` when a SkillCorner ref is requested (a deliberate generation move — the fit corpus now depends on the artifact, which admits the `sound` S1-excluded matches the streamed path silently skipped). The Task-0 producer `scripts/build_skillcorner_s1_event_validity.py` measures the event-side verdict.
- **Four AST rules over EVERY `scripts/*.py` (private included), enforced not documented** (`tests/scripts/_corpus_load_rules.py`): Rule A (no stream loader in a driver), Rule B (`load_match` states `events_only=`), Rule C (no un-sharded LOADING loop), Rule D (unadmitted `events_only` only in the allowlisted edge). `_RULE_A_PENDING` / `_RULE_C_PENDING` ledgers seeded from the live violations at the branch base and DRAINED TO EMPTY as every driver migrated; the population is DERIVED by enumeration over the loader modules (ADR-056 idiom). A `for_each(load=...)` key not equal to `ref.key` needs a `_KEY_EXCEPTIONS` entry proving byte-identical resume.
- **Generation moves (all deliberate, spec §7):** `measure_cover_shadow_argmax_agreement` (token → requested refs + fit `counts_digest`), `measure_rc4_orientation` (key `(provider,)` → `(provider, match_id)`, free — `run_commit` already in its token), and any SkillCorner-including events-only pass (`calibrate_xt_bandwidth`, `calibrate_tracking_defaults`'s held-out xT corpus — admission digest joins the token). Every other driver's shard keys/schema/token are unchanged, so finished generations resume across the migration.
- **New driver args:** `--cache-dir` (raw-artifact cache; `$SILLY_KICKS_CORPUS_CACHE_DIR` default) across the pining/open-data drivers; `--allow-failed` on the xT-fit drivers (refuse-by-default, records failed keys); `--allow-unmeasured` on the events-only SkillCorner passes. `build_sb360_coverage` routes its per-match events+360 fetch through the new `_sb_open_data.fetch_open_360_raw` (JSON-cached), decoupling its resume from statsbombpy's own cache and clearing its pre-existing pyright errors; `_iter_matches` → `_list_match_refs` (refs, no fetch). `validate_gk_decision`'s native-GI pass + both reconstruction legs (SkillCorner fidelity + SB360 sweep) are sharded `for_each` passes; a match without GI artifacts is counted `n_gi_unavailable`, never silently skipped. Decision: PR-S196; ADR-052 (D13/D14/D15 amendment), ADR-102 (count-pass consumer).
...
Match-context-weighted action value (Paul, Klemp & Memmert 2025): `xImpact = VAEP_adjusted × ΔP(win | goal at the pre-action game state)` — a late go-ahead goal scores high, a garbage-time goal ≈ 0 (a trailing equalizer scores low on ΔP(win); see `docs/research/tf63_ximpact/`). Additive, event-only — **no VAEP/tracking retrain, no re-materialize.** `compute_*`, not `add_*` — the action-coupled aggregator count stays 33; +1 C4 container. Two-phase clean-tree provenance: this commit ships the code; the bundled public-corpus weights + the construct-validity report land in the release's 2nd commit (`training_commit` on the clean tree, the TF-53/TF-57/TF-61 2-phase pattern).

- **New self-contained event-only package `silly_kicks/win_probability/`.** An interval-hazard logistic on `(score_diff, minutes_remaining, base_strength, home, man_advantage)` feeds a **forward Markov chain on `score_diff`** (a time- and state-dependent scoring process; Dixon & Robinson 1998, in-game WP application per Robberechts, Van Haaren & Davis 2019). It **replaces, not reuses**, the TF-53 convolution/simplex — team-independence is exactly the approximation the chain removes — so `win_probability` imports no `match_outcome`, no `tracking`, no `vaep` (AST allowlist both directions); `vaep → win_probability` is the only cross-edge and is a lazy function-local import. Public surface: `compute_win_probability` (per-action `p_win`/`p_draw`/`p_loss`/`win_prob_leverage` + conserving `WinProbabilityReport`), `goal_leverage` (per-action `ΔP(win | goal)`, exact chain re-run from `score+1`), `WinProbabilityModel` (pickle-free JSON + SHA256, fail-closed `load`), `WinProbabilityParams`.
- **`VAEP.rate_ximpact`** (+ `vaep/ximpact.py`) — reuses `rate_adjusted` for `VAEP_adjusted`, multiplies by the same-state goal leverage on the acting-team perspective; **raises on HybridVAEP** (inherited), NaN-propagating. In no default xfn list; no `*_xfns` (state-based, reads no post-action outcome).
- **Leverage non-negativity + `score_diff`-monotonicity are gate-enforced, not structural** (a state-dependent mean-reverting kernel is not guaranteed stochastically monotone): a fitted hazard is REJECTED at bundle time by `certify_coherence` — fix the model, never clamp. The exact goal difference and dead-state → 0 ARE structural. **Chirality is N/A** (no geometric/coordinate features) — the fail-closed load guard is SHA256 + the feature-contract probe.
- `win_prob_leverage` glossaried (`feature_glossary`, 455→456); `ximpact` is a VAEP rating-method output (glossary-exempt like `vaep_value`). `p_win`/`p_draw`/`p_loss` base-name-shared with `match_outcome` — glossary definitions generalized to describe both producers. `win_probability` is `metric_contracts`-exempt (per-action feature-grain, not a per-(entity, match) mart family). `NOTICE` attribution; `WinProbabilityParams` frozen (`for_provider` empty, ADR-009). Decision: ADR-101.
## [4.121.0] — 2026-09-20
...
Revival of the un-landed TF-54b counterfactual cone (branch `ab9001c`; the reusable seams landed with ADR-090, the cone itself was not carried). Additive, event-only — **no VAEP/tracking retrain, no re-materialize.** Two-phase clean-tree provenance: the code shipped in the release's first commit (`dd7ed4f`); this commit adds the bundled re-fit weights + validation/census reports (`training_commit` on that clean tree, the TF-53/TF-57/TF-61 2-phase pattern).

- **`territory.compute_territorial_dominance(method="counterfactual")`** (behind the non-default door; `completed_failed` byte-identical). A GSAA-style `(P_complete − outcome)·xT(target)` summed over opponent passes AIMED INTO a defender's trimmed hull — a **failed** pass's intended target is modeled over its death-direction cone (`q`, from the injected xT's transition distribution, family-agnostic) × an injected `PassCompletionModel` (`c`, ADR-022 duck-typed) × the injected `xt` value. 5 cf-only columns (4 metric + `territory_target_source` provenance); conserving `n_target_modeled`/`n_target_unresolved` census (ADR-042). +4 `feature_glossary` metric columns (451→455). `TERRITORY_METHODS = {completed_failed, counterfactual}`. Coexists with the distinct tracking-based `territorial_defense` sibling (ADR-090).
- **`PassCompletionModel` re-fit on the FULL public StatsBomb open-data corpus** (`train_pass_completion.py --all-competitions`: 80 competition-seasons / 3961 matches, `assert_public_corpus`-gated, clean `training_commit`; GroupKFold-by-match OOF **AUC 0.794 / ECE 0.006**). Re-bundled into `silly_kicks/expected_passing/weights/` (was the 4.112.0 WC2022 bundle); pickle-free JSON + SHA256 + feature-contract + chirality, fail-closed load. **Hyrum:** `PassCompletionModel.bundled()` values shift — any consumer of the bundled completion model sees the broad-corpus fit.
- **Defender-ranking census + gate** (`scripts/build_territory_ranking_census.py`, owner-run; ADR-009 apply, a reported artifact, NEVER a library API). Crossed defender+team-cell ICC (`scripts/_crossed_icc.py`, pure-numpy Henderson-III via absorption + a cluster bootstrap) decides whether a per-defender ranking is licensed. **RAN on the full public open-data corpus → LICENSED** (5989 defenders / 1103 multi-team ≥20; defender-share ICC lo=0.121 > 0, power=1.0), so a reported ranking ships as `docs/research/territory_ranking_census/ranking.parquet` (with `census.json` verdict + honest-limit). Reports in `docs/research/territory_ranking_census/` + `docs/research/territory_counterfactual_construct_validity/`.
- **Opt-audit hoists folded in (byte-identical, gated):** batched packing kernel `tracking.compute_packing_metrics_batch` (loop-invariant setup hoisted; scalar delegates n=1; `gk_decision` reconstruction one call/decision); census fit column-prune; duels native `itertuples`; and the crossed-ICC census reduce **full absorption** (no dense `pinv(p)` — the real reduce drops ~7h→27s at n_boot=400, ~920×). See `scripts/_crossed_icc.py` + `tests/scripts/test_crossed_icc_perf_budget.py`.
...
- **New public `silly_kicks.metric_contracts`** — `METRIC_CONTRACTS: dict[str, MetricContract]`, one entry per column-emitting metric family (`team_metrics`, `match_outcome`, `shot_stopping`, `gk_decision`, `territory`, `duels`, `restdefense`) carrying `keys` / `metric_columns` / `columns` / `column_types`. The canonical surface for a consumer schema-drift parity guard — one loop, zero per-package name/type knowledge. A plain `dict` valued by a `MetricContract` `TypedDict` (the house "schemas are plain Python dicts" precedent + pyright-checked field keys, no runtime class). Like `feature_glossary`, it **imports no metric package** — the values are hardcoded literals kept honest by `tests/test_metric_contracts.py`'s round-trip test, so each metric family stays a pure dependency-free leaf and its `test_import_allowlist.py` contract is untouched.
- **Public re-exports** bringing the four asymmetric families in line with the rest: `match_outcome` (`MATCH_OUTCOME_METRIC_COLUMNS`, `MATCH_OUTCOME_KEYS`, `MATCH_OUTCOME_COLUMNS`), `gk_decision` (`GK_DECISION_METRIC_COLUMNS`, `GK_DECISION_SAMPLE_COLUMNS`, and a new `GK_DECISION_KEYS = ("game_id", "keeper")` — the `summarize_gk_decision` grain), `territory` (`TERRITORY_KEYS`), `duels` (`DUEL_KEYS`) — all added to `__all__`.
- **Enforcement** (`tests/test_metric_contracts.py`): per-entry self-consistency, `column_types` coverage (non-None iff the package declares a typed `dict` full-columns mapping), round-trip to the package constants, completeness-by-enumeration (any package exporting a `*_METRIC_COLUMNS` must register; the ADR-056 three-bucket shape, `xsuccess` exempt as a VAEP rating method with no mart column-set, `_UNDERIVABLE` empty), and output-faithfulness (keys/metric ⊆ the package's real declared output).
- **Purely additive** — re-exports + one new key tuple + a new import-free module. No compute/behaviour change, every numeric output byte-identical → **no VAEP/tracking retrain, no re-materialize, C4-free**. `match_outcome`'s `dict` metric constant is unchanged (the registry mirrors its names as a literal). Decision: ADR-098.

## [4.118.0] — 2026-09-17

**TF-53: match-outcome simulation — win probability / xPoints from injected per-shot xG (PR-S189, ADR-097).** New event-only `silly_kicks.match_outcome` package — a hexagonal `compute_*` sibling of `territory`/`duels`/`shot_stopping`/`gk_decision` (imports `spadl` + `id_compat` + `_frame_index.group_rows` only, never `tracking`; AST import-allowlist both directions; nothing imports it). Per `(game_id, team_id)`: injected per-shot xG (port pattern — silly-kicks ships no xG) → each team's **exact Poisson-binomial** goal PMF (DP convolution, **not** `Poisson(ΣxG)`, which discards chance quality) → win/draw/loss simplex + `xpoints = 3·p_win + p_draw` + `expected_goals`. Additive — **no existing feature/model change, no default VAEP retrain, no `*_xfns`**; +5 `feature_glossary` columns (446→451); +1 C4 container (`match_outcome`); the action-coupled `add_*` aggregator count is unchanged (33). Delivered CODE-first in the release's first commit (clean-tree provenance base); the bundled ρ + calibration study land in this second commit (`training_commit` on the clean commit-1 tree, the TF-57/TF-61 2-phase pattern).

- **`silly_kicks.match_outcome` (new event-only package).** `goal_count_pmf(shot_xgs)` (exact Poisson-binomial over per-shot Bernoullis — one 0.8 + eight 0.1 shots share ΣxG but give different win distributions); `match_outcome_probabilities(home_pmf, away_pmf, *, params)` (the two-team scoreline simplex); `compute_match_outcome(actions, *, xg_column, params)` → `(samples, MatchOutcomeReport)`. Own goals counted by RESULT (`result_id == owngoal`, ADR-018) for the Report's scoreline census, excluded from the xG model (they carry no xG). Conservation census (ADR-042: `n_matches_scored + n_matches_excluded_not_two_teams == n_matches_in`); honest-NaN (a team with no shots → `P(0)=1`; a NaN-xg shot excluded + counted; a non-two-team game excluded + counted). Canonical-id grouped (ADR-019), raw ids emitted.
- **Two orthogonal opt-in honesty corrections on a frozen `MatchOutcomeParams`** (house string-dispatch idiom). `same_possession="collapse"` (Rung 3b, within-team, pre-PMF): same-possession shots combine as `1 − ∏(1 − xg)` via `spadl.add_possessions`, so a save→rebound→goal is ONE opportunity, not several. `team_dependence="dixon_coles"` (Rung 3a, cross-team joint): the Dixon & Coles (1997) low-score τ reweighting of the four low cells, with a fitted ρ served from a bundled pickle-free JSON artifact (SHA-256 + plausible-range + `training_commit`, fail-closed load; `functools.cache`'d). `"independent"` on either axis recovers the naive exact Poisson-binomial.
- **DEFAULT = both corrections ON (ADR-097 — measured promotion, a change from the spec's original independent default).** `collapse` on CORRECTNESS (same-possession shots are not independent Bernoulli trials — the independent-shots model double-counts); `dixon_coles` on MEASURED evidence, not the aggregate: the full-corpus calibration study's **paired per-match Brier** shows dixon_coles beats independent on **77.3%** of matches (Wilcoxon p≈7e-125) and `both` beats independent on **68.7%** (p≈3e-71). `"independent"` remains available opt-in on either axis. `MatchOutcomeParams.for_provider` ships EMPTY (ADR-009).
- **Bundled ρ + calibration study (Commit 2, clean-tree provenance).** `scripts/train_match_outcome_dependence.py` fits ρ by scipy `minimize_scalar` MLE on the full **redistributable StatsBomb open-data** corpus (3,961 matches; `assert_statsbomb_open_data_mode` fail-closed public-only) → bundled **ρ = 0.0216** (`training_commit` on commit 1, clean). `scripts/validate_match_outcome_calibration.py` (ADR-052 `for_each` shards, ADR-037 provenance, ADR-056 input contract, reported-not-gated) scores 3-way Brier + calibration slope + xPoints-vs-points per config with a per-fold CV ρ (grouped by `game_id`, held-out, never the bundled weights) → `docs/research/tf53_match_outcome_calibration/`. Calibration: xPoints-bias ≈ −0.008 (well-calibrated), slopes ~0.91, CV ρ stable ([0.033, 0.025, 0.022, 0.010, 0.017] ≈ bundled). **Pappalardo/Wyscout cross-provider calibration is out of scope** — public Wyscout carries no xG, and the metric is xG-driven end-to-end (it would need an xG model silly-kicks does not ship).
- **Attribution** (`NOTICE`, ADR-005): Dixon & Coles (1997); the Poisson-binomial goal model; Sumpter, Soccermatics (module 3) / Twelve match report.

## [4.117.0] — 2026-09-16

**TF-51 Item 4: defensive-credit atomic-SPADL mirror (PR-S188, ADR-096).** The atomic-SPADL mirror of the TF-51 defensive-credit family — `compute_defensive_credits`, `add_defensive_credit`, `compute_bravery` in `silly_kicks.atomic.tracking.features` — **faithful** to the atom stream (delegates on the atom stream like the shipped `_packing_atomic_adapter` / `add_off_ball_run_values` mirrors; no std-parity). **Additive to VAEP for the credit family — no `*_xfns`, no defensive-credit feature change, no retrain of the credit family, no `feature_glossary` growth (the mirror emits already-documented column names); the tracking action-coupled `add_*` aggregator count is unchanged (33), `add_defensive_credit` is the 16th `atomic.tracking` feature mirror.**

- **`_defensive_credit_atomic_adapter`** re-lifts the atom stream into a std-shaped stream: endpoints from `x,y,dx,dy`; std `type_id` for the six rule-anchor types (`pass`/`cross`/`shot`/`shot_penalty`/`take_on`/`bad_touch`, else `non_action`); a **per-type next-atom `result_id`** — pass/cross succeed on a following `receival` or same-team keeper reception; take_on on a retained same-team next atom that is not `interception`/`out`; shot/shot_penalty on a following `goal`; else fail. `possession_id` is derived downstream by `with_possessions` (from time/team, not `result_id`).
- **Shared rollup `_rollup_defending_aggregate`** extracted into `tracking/defensive_credit/_orchestration.py` and called by both the std and atomic aggregates, each assembling on its OWN frame — so the adapter's synthesized columns never leak. **Parity-gated: std `add_defensive_credit` output is byte-identical** (a pure DRY refactor; owner-approved scope).
- **`preserve_native` required-column raise** at the mirror edge — `xg` + `shot_blocked` (credits), `shot_blocked` + `cross_blocked` (bravery) must be threaded through `convert_to_atomic(..., preserve_native=[...])`, else a loud `ValueError` names the missing column and the remedy (ADR-043 — never a silent all-NaN credit column).
- **Bravery honest-NaN set-piece** — atomic `_simplify` collapses `corner_crossed`/`freekick_crossed` into `corner`/`freekick`, so `bravery_set_piece_crosses` = NaN and `n_set_piece_crosses_faced` = `<NA>` (ADR-027 — never a conflated crossed+short overcount); the shots + open-play-cross headline (`bravery_pct_known_domain`) is exact.
- **Faithful-representation limitations (documented, pinned):** set-piece crossed/short indistinguishable → bravery set-piece NaN; a `shot_freekick` collapses to `freekick` → not detected as a resulting shot (`{shot, shot_penalty}` only on atomic); window params count atom rows (denser).

**Co-shipped fix (BREAKING for AtomicVAEP consumers):** removed the duplicate `"interception"` from `atomic/spadl/config.py::actiontypes` — it was inherited from std at idx 10 but re-appended, so the reverse dict resolved `actiontype_id["interception"] == 24`, silently shadowing the std interception events `convert_to_atomic` keeps unremapped. It now unifies at idx 10 and the atomic-only tail renumbers down by one (`out` 25→24, `offside` 26→25, `goal` 27→26, `owngoal` 28→27, `yellow_card` 29→28, `red_card` 30→29, `corner` 31→30, `freekick` 32→31). All code consumers resolve ids symbolically and auto-follow; `len(atomic.actiontypes)` 33→32 (C4 `silly_kicks.atomic` container prose "33-type"→"32-type", html regenerated via Graphviz `dot`). **Migration owed:** the atomic `type_id` renumber changes the **default** `AtomicVAEP` feature encoding (`xfns_default` = `fs.actiontype` raw `type_id` + `fs.actiontype_onehot` position-enumerated), so any persisted/champion AtomicVAEP consuming the default xfns must be **retrained** on the new encoding (or proven not deployed), **paired with an atomic-SPADL re-materialize** — a re-materialize WITHOUT the retrain is a silent old-id-weights-vs-new-id-features skew (no gate). silly-kicks ships no bundled AtomicVAEP weights (fit consumer-side), so this obligation lands on the lakehouse. The committed `tests/datasets/spadl/atomic_spadl.json` fixture was surgically renumbered (only the tail `type_id`s move; proven purely the renumber).

Ships **no `*_xfns`** (F4 result-leakage, inherited from v1). Decision: ADR-096.

## [4.116.0] — 2026-09-15

**TF-61: xSuccess (event-only, END-BLIND action-completion) + VAEP_adjusted (outcome-bias-free VAEP) (PR-S187, ADR-095).** Two complementary event-only primitives from Paul, Klemp & Memmert (2025), "Beyond Outcome Bias" (MLSA 2025, paper 225): a bundled action-completion model and a risk-aware VAEP rating that removes **outcome bias** by weighting the score/concede deltas by completion probability. **Additive — no existing model/feature change, no default VAEP retrain, no `*_xfns`, no `feature_glossary` growth; +1 C4 container (`xsuccess`); the action-coupled `add_*` aggregator count is unchanged (33).** Delivered CODE-first (this commit); the bundled xSuccess weights + the construct-validity report land in the release's second, clean-tree-provenance commit (`training_commit` requires a clean tree, ADR-052 — the TF-57/TF-62 2-phase pattern).

- **`silly_kicks.xsuccess` (new event-only package).** `XSuccessModel`: calibrated XGBoost `P(success | pre-action context)` over ALL on-ball action types (`result_id == success` label; `non_action` dropped). **END-BLIND (correctness, not a caveat):** features are start-anchored geometry + type + bodypart + time — **never** the realized action end. For a *failed* action the SPADL end IS the outcome (the interception/death point), so an end-using completion model is a **postdiction** (target leakage) that would re-inject the very bias VAEP_adjusted removes; the `PassCompletionModel` precedent does not transfer (it serves on a caller-*supplied* target, not a realized outcome). Guarded by an **end-location invariance** test (perturb `end`/`result` → features byte-identical, the `ReceiverModel` bar). Served via xgboost (`[xgboost]` extra, the `XShotOccurrenceModel` precedent) + pure-numpy isotonic — no runtime sklearn; pickle-free booster-JSON + metadata + SHA256, fail-closed `load` (SHA → feature-contract → chirality; ADR-011/016/040/050). A **per-type-logistic fallback family** (`fit(family="per_type_logistic")` / `train_xsuccess --family`) is the ADR-009 calibration fallback — one standardized logistic per `action_type` on the shared feature set, calibrated-by-construction, pure-numpy serve, one-sided/thin types serve the per-type base rate. Import-allowlist gate: imports only `spadl`/`id_compat`/numpy/pandas (xgboost/sklearn function-local), **never `tracking`**; nothing imports `xsuccess` except `vaep`.
- **`VAEP.rate_adjusted` + `silly_kicks.vaep.adjusted.adjusted_value` (new).** Re-scores an **already-fitted STANDARD VAEP** on a **surgical result-feature counterfactual** — flips ONLY the current action's (a0) `result_onehot`/`actiontype_result_onehot` to success/fail, holding locations, goalscore, and the predecessors' real results fixed (the gold-standard ceteris-paribus form, *not* the paper's whole-dataset flip, which inflates goalscore + rewrites predecessor history) — weights by `xSuccess`/`(1−xSuccess)`, and runs the *existing* `formula.value` delta (eqs 8–12). **No retrain of any model.** Requires result-bearing features: on `HybridVAEP` the flip is a no-op and `rate_adjusted` **raises** (non-vacuity guard); a NaN `xSuccess` → NaN adjusted value (never fabricated); frames threaded to every counterfactual build.
- **Full ruthless-efficiency stack (dev/train-time).** `XSuccessObjective` (ruthless `CachedObjective`, the `XShotOccurrenceObjective` template; StratifiedGroupKFold held-out log-loss, `assert_cache_equivalence` < 1e-9) for XGBoost HPO; `scripts/evolve_xsuccess_features.py` (OpenEvolve feature-representation discovery within a leakage-safe, END-FREE input allowlist — the winning representation is committed deterministic code with clean provenance + a held-out check + the end-blind evaluator; seed-fallback ships the seed if nothing beats it — **Stage A shipped the seed** (XGBoost gain null; a real logistic-family gain is deferred to the Commit-2 family gate; `docs/research/xsuccess_vaep_adjusted/`)); `scripts/train_xsuccess.py` (public StatsBomb open-data corpus via `for_each` shards, ADR-052; calibrated fit + OOF metrics + MODEL_CARD + provenance, ADR-056); `scripts/publish_xsuccess.py` (the ADR-088 card-required HF seam).
- **Corpus.** Bundled weights train on the full **redistributable StatsBomb open-data** corpus (sourced only via the open-data loader `load_open_data_matches` — redistributable by construction; the `train_pass_completion` convention), not WC2022-only — more data for rare-type calibration. silly-kicks ships no xG; StatsBomb's own xG is injected for the VAEP_adjusted aggregate-vs-xG validation only.
- **Bundled `default` weights + validation (Commit 2, clean-tree provenance).** The calibrated XGBoost is bundled (`silly_kicks/xsuccess/weights/`, `training_commit` on the code commit) — trained on the **full redistributable StatsBomb open-data corpus (3,961 matches / 7,974,436 on-ball actions, 80 comp:season pairs)**, Optuna HPO 30 trials; GroupKFold-by-match OOF **AUC 0.895 / Brier 0.084** (base 0.835). **§9.1 family gate PASS** (calibration-in-the-large ratio 1.0000; per-type reliability max |pred−obs| 0.006, mean 0.001 across 21 types) → the XGBoost family ships (the per-type-logistic fallback is not triggered). Stage-A OpenEvolve **shipped the seed** representation (XGBoost gain null; a real per-type-logistic-family gain deferred to the family gate — moot here since it passed); provenance + both evolve winners in `docs/research/xsuccess_vaep_adjusted/`. **§9.2 VAEP_adjusted validation** (WC2022, StatsBomb xG injected): outcome-bias reduction — corr(value, realized success) **0.154 → 0.004**, success−fail gap 0.0159 → 0.0003 — while per-player shot-value↔xG Pearson **improves 0.423 → 0.547** (tracks expected quality, decouples from realized goals). Reported-not-gated (ADR-009).
- **Attribution** (`NOTICE`, ADR-005): Paul/Klemp/Memmert 2025; Anzer–Bauer *Expected Passes*; von Neumann–Morgenstern / Bernoulli expected-utility; XGBoost (Chen & Guestrin 2016); OpenEvolve/AlphaEvolve (offline feature search).

## [4.115.0] — 2026-09-15

**TF-52: event-only team-KPI module `silly_kicks.team_metrics` (PR-S186, ADR-094).** A new event-only `compute_*` sibling (next to `territory` / `duels` / `shot_stopping` / `gk_decision`) emitting per-`(game_id, team_id)` team match-report KPIs from SPADL events alone — the Twelve.football glossary v1 set plus the events-based practitioner v2 items (MSC Bootcamp / Tigres-Clemson). `compute_team_kpis(actions, *, xg_column=None, params) → (samples, TeamKpiReport)`; imports `spadl` + `id_compat` ONLY (AST import-allowlist gate, nothing imports it); canonical-id grouped (ADR-019); a non-two-team match is excluded-and-counted (`TeamKpiReport`, ADR-042). Additive — **no VAEP/tracking retrain, no re-materialize**; a `compute_*`, NOT an `add_*` (C4 aggregator count stays 33).

- **Three KPI families over one possession foundation.** The module consumes `spadl.add_possessions` (the assumed TF-52-owned segmenter already existed) once, then assembles: **pressing** (`_pressing.py` — PPDA, defensive intensity, time-to-defensive-action/-recovery, recoveries + within-Ns %, counter-press regains), **progression** (`_progression.py` — field tilt, pass tempo, long-ball %, the three line heights, the conversion chain, final-third entries, shots, high-opportunity shots, breakout-by-channel, possessions-retained-after-Ns), and **build-up** (`_buildup.py` — the 6-state build-up outcome taxonomy, post-regain security, switch-conditioned press success). Own-touch KPIs use each team's own action-LTR frame; the PPDA opponent-frame denominator is library-owned via `_orientation.py` (`silly_kicks.reflection`, ADR-028/045).
- **The within-Ns post-recovery companion is the transition-output block (owner-ratified Option B).** §4.5 specified "the offensive metrics" without enumerating them; the owner ratified the set as the four per-action COUNTS `final_third_entries` / `box_touches` / `shots` / `high_opportunity_shots`, re-computed inside `post_recovery_window_seconds` of a recovery. This added **two new base KPIs**: a plain `shots` count (the module previously had only the xG-gated `high_opportunity_shots` — the transition family is now xG-independent) and a raw `final_third_entries` count (per-action ball-crossing of `x = 2·field_length/3`, complementing the possession-rate `poss_to_final_third_pct`). A *breakout* is EXCLUDED on correctness grounds (one channel per possession → no per-action-window restriction), as are possession-rates — correctness exclusions, not scope cuts.
- **Frozen `TeamKpiParams`, `for_provider` EMPTY (ADR-009).** Geometry defaults from `spadlconfig` (ADR-050); the counter-press window is a seconds-XOR-passes `CounterpressWindow` value object (`__post_init__`-enforced) with documented `COUNTERPRESS_PRESETS`. Per-provider overrides ship empty until a separate gated apply-PR.
- **Public-corpus reliability driver — reported-not-gated.** `scripts/validate_team_kpi_reliability.py` runs the KPIs over **public** event corpora — StatsBomb open data (defaulting to the **entire open-data manifest**, `all_open_competitions()`, thousands of matches; a single tournament is far too thin for a reliability ICC/split-half) + the public Wyscout / Pappalardo 2019 set — and writes an aggregate, reproducible report with three legs: per-KPI **reliability** (team-discrimination ICC(1) + odd/even split-half + Type-II slope), **possession-foundation ground truth** (`add_possessions` boundary recall/precision/F1 vs the provider's native possession id, threaded via a new `preserve_native` kwarg on `scripts/_sb_open_data.load_open_data_matches` — which also attaches StatsBomb's own `statsbomb_xg` by `original_event_id` so the xG-gated `high_opportunity_shots` KPI is measured rather than all-NaN), and per-provider **comparability** (`--compare`; a KPI is flagged poolable only where both providers report finite same-sign ICC within a tolerance). ADR-052 `for_each` shard driver + ADR-037 clean-tree provenance + ADR-056 input contract. **Public-only is FAIL-CLOSED, corpus-appropriately (ADR-038, NOT the pining `assert_public_corpus`, whose 27-match registry cannot represent open data):** the StatsBomb leg refuses to run with credentials set (`assert_statsbomb_open_data_mode`; no creds ⇒ open-data manifest only), the Wyscout leg allowlists the seven public Pappalardo competitions. The pure stat/shaping kernels are CI-tested, the corpus orchestration is owner-run. Recommends, never applies (ADR-009).
- **Glossary + vocab.** +44 feature-glossary columns (402 → **446**); `docs/c4/architecture.{dsl,html}` re-rendered via Graphviz `dot` (+1 `team_metrics` container). `feature_glossary.Unit` gained two rate literals (`passes/min`, `actions/min`) for `pass_tempo` / `defensive_intensity` — an additive closed-vocab extension, no `GLOSSARY_SCHEMA_VERSION` shape change. `NOTICE` attribution per ADR-005.
- **Scope.** The tracking-flavoured v2 items (interception-height relative to the opponent line; compactness-recovery-time) and aerial first→second-ball chaining are OUT of this event-only module — the first two by the owner's event-only scope, the third because SPADL encodes no aerial action type.

## [4.114.0] — 2026-09-13

**TF-57: Event↔tracking synchronization via extended Needleman–Wunsch (ELASTIC v2) replaces the greedy TF-43 aligner (PR-S185, ADR-093).** `tracking/_elastic_sync.py` is rewritten around a clean-room reimplementation of ELASTIC v2 (Kim et al., CIKM 2026, arXiv:2608.30227) — a **global, order-preserving extended-Needleman–Wunsch alignment** between the virtual-termination-enriched SPADL event sequence and sparse ball-touch candidate frames, run per **in-play (`ball_state=="alive"`) episode** (scipy peak detection + a numpy DP; no neural nets). The MPL-2.0 reference code is **not** used (clean-room from the paper; the PathCRF precedent). The greedy MLSA-2025 algorithm is **removed**.

- **Measured (DGX, 3 CC-BY Sportec matches, our DFL tracking + an approximate `home_N→DFL` vote-map — the *same* conditions as the greedy's 8.2 % W2):** **within-2 (the paper's primary W2) 0.862** (per-match 0.848 / 0.847 / 0.891 — all folds improved; min-fold 0.847), **≈ 11× the greedy aligner**; committed CC-BY oracle slice (n=67, a J03WMX span that covers a goal) start exact 0.493 / within-2 0.791 / within-25 0.955, reception within-5 0.922. Three cumulative clean-room wins over the as-built NW: **central-difference acceleration** (paper Eq. 3 — the double-backward diff peaked the accel one frame late; oracle exact 0.335→0.583), **in-play episode grouping** (the paper's consecutive-alive-frame episodes replace the `add_possessions` proxy; W2 0.845→0.856), and an **OpenEvolve-tuned `_score`** (reweight toward proximity, dampen the decoy-inflated accel/kick-distance, + a directional depart/arrive slope term; W2 0.856→0.862, white-box score-peak 0.728→0.745, all three folds up, no regression).
- **The residual to the paper's same-source NW (W2 0.965) is a CROSS-SOURCE event-time-jitter ceiling — NOT the alignment algorithm or player identity.** The paper (arXiv:2608.30227) explicitly adopts **W2 (within-2-frames, 0.08 s) as its PRIMARY metric** and reports **no exact-frame headline** (Table 2, event start: ELASTIC-NW **96.5 %** W2, ELASTIC-Greedy **84.1 %** W2, on the paper's own same-source data). Our cross-source **W2 0.862** sits **ABOVE the paper's greedy** (0.841) and **~10 pts below its same-source NW** (0.965). *(This corrects the rev-3 record, which set an "88.4 % exact" acceptance target — a metric misreading: the paper has no exact-frame headline, and 88.4 % is not its NW figure; the authoritative comparison is on W2. Verified against the paper as read, 2026-09-14.)* The oracle scores Kim's re-annotated event labels against our DFL-OBJ tracking via a per-period vote-map, leaving ~0.8 s of per-event event-vs-tracking jitter (median anchor error ~20 frames, uncorrelated across neighbours). An exhaustive lever investigation — scoring (OpenEvolve, converged early), ball height, velocity-direction, a time-proximity prior, and a local time re-anchor — found each tapped, weak, or dead (the anchor error is per-event jitter, not drift, so a local re-anchor cannot help). Same-source (a provider's own events + tracking) alignment should approach the paper's same-source figure.
- **Scoring provenance (clean-room boundary, ADR-093).** The NW algorithm, the ELASTIC scoring categories, the candidate-detection features, and the `_score` **feature set** are re-derived from the paper — never the MPL-2.0 reference code. The `_score` **weights** (`w_ba/w_pbd/w_kd/w_dyn` = 0.6/1.6/0.8/1.0 + a directional depart/arrive slope term) are **OpenEvolve-tuned** on the 3-match CC-BY corpus and are **owner-approved (2026-09-14)**: the paper-faithful uniform-weight path had plateaued ("nowhere near" the paper at the greedy/early-NW start), and the tuning is a small, removable lift (revert-to-uniform costs ≈0.05 oracle-exact / 0.006 W2, gates stay green). The tuning is a data-fit, **not** an MPL-source read. The OpenEvolve run provenance (objective/folds/seeds/config) + a **held-out** (leave-one-match-out) validation landed in the paired **provenance commit** (`docs/research/tf57_elastic_scoring_tuning/`, run clean-tree on the DGX against the feature commit): **mean held-out W2 0.860 ≈ in-sample 0.862**, confirming the ship bar is not in-sample-inflated (ADR-093 §16.9/§16.11). `test_dataclass_defaults_are_paper_intent_set_constants` covers only the `ElasticSyncParams` dataclass fields (which ARE the paper's intent-set constants); the tuned discriminative weights live in `_score` and are documented AS tuned.
- **Two surfaces, one pure engine.** The elastic mart producer (`align_events_to_frames` / `add_elastic_sync` / `elastic_sync_xfns` + atomic mirror) keeps `elastic_frame_id`/`elastic_confidence`/`elastic_error_seconds` (**values move**) and **adds** the jointly-detected reception columns `elastic_receive_frame_id`/`_confidence`/`_error_seconds`. A new `link_actions_to_frames_elastic → (pointers, LinkReport)` exposes NW alignment under the canonical ADR-004 contract — a drop-in for the `links=` kwarg across the `add_*` family. The guarded time-based `link_actions_to_frames` (ADR-004/017) is **untouched and remains the default** (this is an alternative strategy, not a replacement); its behaviour is byte-identical — the new opt-in `suppress_time_base_hint` flag on the shared low-coverage guard defaults off.
- **Continuous-only, enforced.** ELASTIC-NW needs a dense ball trajectory for candidate detection; on declared velocity-unavailable-by-design freeze-frames (`validate_velocity_regime == POSITIONAL_ONLY`, e.g. StatsBomb-360) `align_events_to_frames` returns an **honest empty alignment (all-NaN)** rather than fabricate a sync from disconnected snapshots (ADR-063 discipline — a *declared*-unavailable regime degrades honestly, it does not crash a mixed-provider pipeline; a velocity-less-but-*continuous* provider is `VELOCITY_MISSING`, not `POSITIONAL_ONLY`, and still runs, since the engine uses positions not `vx`/`vy`). This corrects the greedy path's silent spurious values on freeze-frames; the SB360 audit now records `honest_nan`.
- **Validation.** A committed CC-BY oracle slice (`tests/datasets/elastic_sync/j03wmx_slice/`, derived under CC BY 4.0 from the Sportec Open DFL dataset / Bassek et al. 2025 + the ELASTIC re-annotation) + a CI regression gate scoring NW vs a from-scratch `|pred − gt| ≤ N` metric (never ELASTIC's MPL metric), with the floors recalibrated to the achieved-minus-margin numbers. The white-box **score-peak gate** is a regression bound (`≤ 26` offenders on the oracle; 0 is unreachable clean-room — the cross-source-jitter ceiling). ADR-019 id-dtype invariance re-guarded on the actor-membership join (the exact seam of the historical constant-0.6 bug); ADR-073 scale guard on the candidate detection + episode loop.
- **Hyrum / retrain (intended).** `elastic_*` mart values move + 3 new columns → **lakehouse `elastic_*` re-materialize**. `elastic_sync_xfns` is in **no** default xfn list → **no default-config VAEP retrain**; a consumer that opted those columns in retrains those models. **Breaking:** `ElasticSyncParams` + `add_elastic_sync`'s keyword signature change (greedy weights removed; no shim, fail-loud). No new `add_*` aggregator (C4 aggregator count stays 33), but the 3 new `elastic_receive_*` columns bump the glossary feature-column count 399→402, so `docs/c4/architecture.{dsl,html}` are re-rendered (Graphviz `dot`). Attribution updated in `NOTICE` + `feature_glossary`.
- **Dependency.** `ruthless-efficiency[optuna]` floor raised **0.4.0 → 0.6.0** across `[calibration]`/`[test]`/`[train]` to track the current first-party release (0.6.0, published 2026-09-13); `uv.lock` re-resolved (117 packages). Verified compatible against the full ruthless API surface silly-kicks consumes — `tests/calibration/` (Candidate/Metrics/Direction/`assert_cache_equivalence`), the xShot/xCross Optuna integration, and the driver-fingerprint (`for_each` → `ruthless.fingerprint`) tests all pass on a clean 0.6.0 resolve; **no source changes required.**

## [4.113.0] — 2026-09-12

**TF-62: GK build-up Decision-quality metric — chosen-vs-available (`gk_decision`) (PR-S184, ADR-092).** A new sibling package `silly_kicks.gk_decision` reframes xT-GK from possession **value** (quarantined — v1 degenerate per ADR-024; v2 not construct-validated per ADR-036; ~80 % team-confounded per the Eyestone collaboration, "ranking not licensed") to **decision quality**: `decision_value = value(chosen) − E[value(available options)]`. Scoring the keeper's choice against the option set they **could** have played **normalizes out the team-created option set by construction** — a keeper is credited only for *which* reachable option they pick, not for the options existing. **Additive — a `compute_*`, NOT an `add_*` (the 33 action-coupled aggregator count is unchanged); in NO default xfn list; no VAEP/tracking retrain, no re-materialize; +1 C4 container (`gk_decision`); +5 feature-glossary columns (394 → 399).** Native (SkillCorner GI) + reconstruction (SB360 + full-tracking) tiers ship as ONE coherent code commit — native-first is a BUILD ORDER, not two code commits (the metric reuses the already-bundled `PassCompletionModel`, so no weight-provenance split); the construct-validity report lands in a SEPARATE provenance-mandated commit (clean-tree re-run, `run_tree_dirty: false`, ADR-090 pattern), so two commits total (ADR-092 §12.7).

- **The metric.** Per GK build-up decision (the `gk_distribution_mask` domain — a goal-kick or an acting-GK open-play pass) with **≥ 3 options**, option value `EV(o) = xpass_completion(o) × (1 + max(0, opponents_bypassed(o)))` (completion × progression — the Eyestone-validated form; **deliberately NOT** `xthreat`, degenerate for keepers ~0.0005, and **NOT** `passing_option_score`, a safety score ~0-correlated with value). Emitted per decision: `decision_value` (chosen − mean available; random ⇒ ~0), `sel_efficiency` (chosen / max), `decision_pct` (fraction of alternatives the chosen option beats, **ties split half** ⇒ random = 0.5 exactly — the spike's strongest discriminator), `chosen_ev`, `best_ev`, `n_options`, `option_set_source`. `summarize_gk_decision` rolls up per `(keeper, game_id)`.
- **Hexagonal `OptionSet` port.** The provider-specific complexity is isolated behind ONE port that yields a **uniform option-rows schema** (`is_chosen`/`completion`/`opponents_bypassed`/`option_set_source` per candidate), so the scoring engine + `option_value` are tier-agnostic. `SkillCornerGIOptionSet` (native — reads the curated `passing_option` rows with native `opponents_bypassed`; **no** packing/goal-map) and `ReconstructedOptionSet` (SB360 + full-tracking — the injected `PassCompletionModel` xPass per candidate + the reused `compute_packing_metrics` progression + a reachability filter, every option scored in action-LTR) both yield that schema.
- **New `providers/skillcorner` GI parse port** (`parse_passing_options`), a testable provider extractor (ADR-084 `appearances.py` pattern; shaping only — raw GI loading stays scripts-side). The native tier imports only `silly_kicks.id_compat` + pandas/numpy (the parsed option rows are injected); the reconstruction tier imports `silly_kicks.tracking` PUBLIC seams (`compute_packing_metrics` / `action_ltr_goal_map` / linking) + `keeper_identity` + `reflection` — never a `silly_kicks.tracking._*` private (AST import-allowlist both directions; **nothing imports `gk_decision`**).
- **Spike GO (the gate that licensed the build).** An owner-run spike (2026-09-11, owner-tier corpus, non-reversible aggregate statistics only) established, on the native tier (906 matches / 22,860 GK decisions / 166 keepers): **responsive** (`sel_efficiency` 0.700 vs a random-choice placebo 0.661, `t = +21.6`; `decision_pct` 0.557 vs 0.50, `t = +23.3`), **discriminating** (one-way keeper ICC 0.029–0.039 vs a keeper-label permutation null, `p = 0.000`), **net of team** (club-adjusted leave-one-keeper-out keeper ICC 0.09–0.15, `p = 0.000`) — reproducing Eyestone's "real, modest, largely-but-not-entirely-team" verdict at scale. The transfer leg (11 usable crossing keepers) was **inconclusive** → **ranking not licensed** (measured, not assumed). The reconstruction path is xPass-faithful (Spearman ρ 0.746 completion) **conditional on a reachability filter** (the load-bearing PR2 finding: "all visible teammates" measurably inverts the signal on zero-velocity SB360).
- **Owner-run construct-validity battery** `scripts/validate_gk_decision.py` (ADR-052 shards, ADR-037 clean-tree, ADR-056 input contract) — **RAN on the DGX** over the owner-tier corpus (aggregate-only, reversibility-not-provenance; report `docs/research/gk_decision_construct_validity/`). **Native (full 906-match corpus / 22,934 decisions / 153 keepers): VALIDATED as an instrument** — responsive (`decision_pct` t≈+27), discriminating (keeper ICC 0.03–0.04 vs a permutation null ~0.001, p=0), net-of-team real (club-adjusted ICC 0.07, p=0), transfer inconclusive (11 usable crossing keepers) → **ranking not licensed**. **Reconstruction fidelity** (SkillCorner Rosetta Stone, per `(keeper, game_id)`) at the shipped default: sel-eff ρ ≈ **0.24**, decision_value ρ ≈ **0.25** (both p<0.001, n=1,764 pairs over 875 S1-passing SkillCorner matches) — moderate, significant. **The reachability sweep is a threshold GRID**: SB360 INVERTS at reachability 0.5 (the bundled WC2022 xPass runs generous on SB360 — 95% of reconstructed options ≥0.5, so a 0.5 filter can't bite) and is responsive at ~0.85, so **the shipped default is 0.85** — 0.5 was a useless default (inert on the native tier, inverting on SB360). A deeper per-provider xPass recalibration for SB360 remains a future **ADR-009** refinement (NOT required for responsiveness — the 0.85 default handles it). **Reported-not-gated, promotes NO default.**
- **HONEST LIMIT (load-bearing, in the module docstrings, glossary `definition`s, `NOTICE`, and the battery report):** the metric is validated as an **instrument**, **NOT** player-attributable — the net-of-team signal is real within multi-keeper teams but the transfer-robust keeper-intrinsic component is underpowered/unconfirmed; **per-keeper numbers are NOT a ranking**. Ranking is a future ADR-009 gated on a crossed keeper+team ICC over a larger multi-club transfer corpus (same posture as ADR-090). The metric is also **value-function dependent** (`completion × progression` penalises deliberately-direct keepers even net of team; the pluggable typed value-fn seam is the hook — xT-/retention-based variants are reserved-typed-not-built).
- **Governance.** `GkDecisionParams` frozen (`reachability_min_xpass = 0.85` default — the responsive "reachable = high-completion" value, since 0.5 inverts on SB360; `for_provider`-tunable; `min_options = 3`; the value-fn selector). Conserving `GkDecisionReport` (`n_decisions_scored + Σ drop_reasons == n_decisions_in`, ADR-042/043; drops `too_few_options`/`no_unique_chosen`/`chosen_unvalued` — never a fabricated 0). Keepers grouped on the CANONICAL id, raw id emitted (ADR-019/ADR-085). The metric never reads `result_id` or any post-contact outcome (value = pre-outcome xPass + geometry), so it ships **no `*_xfns`** (no leakage concern). `feature_glossary` entry per emitted column (ADR-048); `NOTICE` attribution (ADR-005). The bundled xPass is public-corpus-trained; the battery's owner-tier numbers are non-reversible aggregates.
- **Reconstruction tiers + the promoted goal-map seam.** `ReconstructedOptionSet` scores SB360 freeze-frames + full-tracking in action-LTR (match-LTR frames reflected via the ADR-045 seam; the actual pass end is the chosen option, **never snapped**; the nearest teammate within `receiver_exclusion_m` of the end is the presumed receiver, excluded from the *alternatives* only). The public **`tracking.action_ltr_goal_map`** constructor is **promoted** (ADR-055 "ONE implementation of the defended goal end" + the ADR-091 per-action-LTR convention) and `territorial_defense`'s private copy is **migrated onto it (duplicate deleted)**. Conservation grows two reconstruction drop reasons (`no_frame` / `fov_cropped`, ADR-042/077); `compute_gk_decision_value` auto-pulls them from the adapter's `drop_counts()` (after `option_rows()` runs) so `n_decisions_in` stays the true decision population. The reconstruction was fidelity-validated in-cycle (moderate, significant — sel-eff ρ ≈ 0.24 at the shipped default) by the Rosetta-Stone leg; the SB360 tier is responsive at the shipped default reachability 0.85 (the sweep confirms 0.5 inverts on SB360 — why the default is 0.85, not the useless 0.5). The `gk_decision` import-allowlist relaxes to the tracking-CONSUMING-sibling shape (tracking PUBLIC allowed, `tracking._*` banned).

## [4.112.0] — 2026-09-11

**TF-54b (re-scoped): SB360 territorial-defense counterfactual — how much a defender's *positioning* suppresses the attacking team's threat (PR-S183, ADR-090).** A new tracking-consuming `silly_kicks.territorial_defense` package measures threat suppression via a **model-free removal (marginal-contribution) counterfactual** on StatsBomb-360 freeze-frames: `threat_suppressed = compute_threat_pc(counterfactual) − compute_threat_pc(actual)`, where the counterfactual is the factual frame with **one defender row removed** (pitch control re-partitions the vacated space to the remaining players — Fernández–Bornn marginal player value). **Additive — no VAEP/tracking retrain, no re-materialize; +2 C4 containers (`territorial_defense` + `expected_passing`, the latter new-to-`main`); the 33 action-coupled `add_*` count is unchanged (this is a `compute_*`).** Two commits (this library code + release; owner-run PassCompletionModel weights + construct-validity battery, merged non-squash to preserve the `training_commit` provenance chain).

- **Sign convention: attacker-value units, `positive = threat suppressed`** — INVERTED from gkdv's "negative = deterrent".
- **Two arms.** **Arm A** (`a_threat_suppressed`) — action-anchored, **identity-EXACT** via a new actor identity bridge, over the acting defender's own defensive actions (tackle/interception/clearance, `type_id ∈ {9, 10, 18}`). **Arm B** (`b_threat_suppressed`) — hull-based, attribution-**approximate**, nearest-to-target defender removed (receiver-lane evaluated in the owner-run battery), single-sourcing the v1 `territory` trimmed hull via a configurable `own_half_max_x` (default midfield, matching `territory`); reports `b_attribution_slippage` (the rate the position-chosen contesting defender is **NOT** the hull-owner D — attribution error, **lower = tighter**; honest-NaN, never a fabricated 0, on anonymous SB360 frames).
- **Actor identity bridge.** SB360 freeze-frames are anonymous (`snapshot_to_tracking_frames` numbers the rows). `is_actor` is re-plumbed through the StatsBomb port (`shape_snapshots`) and `snapshot_to_tracking_frames` as a **snapshot-only** extension column (base `TRACKING_FRAMES_COLUMNS` unchanged); `silly_kicks.keeper_identity.apply_actor_identities_to_frames` stamps the acting player's real id onto the anonymous actor row (shared home next to `apply_keeper_identities_to_frames`; ADR-078 bridge pattern, ADR-084 module home). Independently useful (a concrete second consumer is the eyestone GK build-up-decision metric).
- **`pitch_control_method="spearman"` is a HARD `__post_init__` constraint** (a GK-blind method is unrepresentable, mirroring `GkdvParams`); the arms accept **no `pitch_control_cache`** (ADR-043 landmine: the identity-keyed cache would serve the factual surface to the counterfactual and collapse every delta to exactly 0).
- **Conservation + honest-NaN (ADR-042/ADR-027/ADR-055/ADR-063/ADR-077).** A removal on a defender-depleted frame (`< min_defenders_after_removal`, default 1) is **dropped-and-counted** as `removal_undersupported` (a 0-defender surface degrades to attacker-controls-all — an upward-biased outlier); an unresolvable goal end → `unresolved_geometry` honest-NaN (this cycle fixed a **real ADR-055 crash**: `compute_threat_pc` raised `GoalEndUnresolvedError` on keeperless freeze-frames — the arms now catch it at the edge); an under-observed local region → `fov_cropped_local`. The depletion guard applies to **both arms**, and `TerritorialDefenseReport` conserves both — Arm A over its domain (`n_frames_scored + Σ drops == n_frames_in`) and Arm B over its qualifying `(defender, in-hull-pass)` pairs (`arm_b_n_scored + Σ arm_b_drop_reasons == arm_b_n_in`). Threat suppression is a Tier-1 dimensionless lift at zero velocity (ADR-063).
- **Per-action goal resolution, dual-mode via `frame_convention` (ADR-091).** SB360 freeze-frames are per-acting-team-LTR (ADR-028), so a per-MATCH `resolve_defended_goals` (mean GK x) is bimodal and scored the metric at **~0 on real SB360** (a pre-commit WC2022 smoke caught it — every frame dropped `unresolved_geometry`). The default `frame_convention="per_action_ltr"` now resolves the goal **per frame** from the action-LTR convention (`action_ltr_goal_map`: the acting team attacks x=105) — deterministic, **GK-independent/FOV-proof** (keeperless frames still score) — while `frame_convention="match_ltr"` reuses `resolve_defended_goals` for continuous-tracking-derived frames (byte-identical to the historical behaviour). The compute owns the convention via a `goal_map_for` factory; `classify_arm_a_domain`/arms are convention-agnostic. **Additive — no re-materialize, no VAEP retrain** (the metric never produced committed output; `match_ltr` is byte-identical). A convention-faithful `make_per_action_ltr_fixture` + a two-sided regression are the guard: the match-oriented e2e/audit fixtures had **masked** the defect.
- **HONEST LIMIT — NOT construct-validated (see the battery result below), and per-defender numbers are NOT player-attributable.** The marginal-removal delta is **team-conditioned by construction** (vacated space re-partitions to teammates); the validation corpus has **zero cross-team defender observations**, so per-defender numbers are **NOT a defender ranking** and the elite-defender ("Van Dijk") prior is elite-defender/elite-team **collinear** (FACE-VALIDITY, not attribution). Ranking is a future ADR-009 gated on a crossed defender+team ICC over a multi-club transfer corpus.
- **`territory`'s reserved `counterfactual` door REMOVED** (`TERRITORY_METHODS → {"completed_failed"}`; the `NotImplementedError` branch deleted) — the counterfactual "prevented" valuation now lives in `territorial_defense`. `completed_failed` output is **byte-identical**.
- **Kept seams landed as reusable infrastructure** (carried from the unmerged `ab9001c`; the event-only *cone* was NOT carried): `expected_passing.PassCompletionModel` (event-only logistic; pickle-free JSON+SHA256; fail-closed load), `xthreat.destination_profiles` + `xthreat/_counterfactual_seam.py`, `scripts/train_pass_completion.py`, `scripts/_sb_open_data.py`. **Commit 2 bundles the `PassCompletionModel` default weights** (`silly_kicks/expected_passing/weights/`, trained on public StatsBomb open-data WC2022 — GroupKFold-by-match OOF AUC 0.811 / ECE 0.007 / Brier 0.105 vs base rate 0.844, clean `training_commit`); `PassCompletionModel.bundled()` now serves them and the `bundled()` test runs. The model card describes the model as a **reusable seam** — the event-only "threat prevented" counterfactual cone consumer it was built to weight was not carried into this cycle, so no shipped feature consumes it yet (it is library infrastructure for a future consumer).
- **Governance + demotion:** the metric shipped in commit 1 with 3 `feature_glossary` columns, a `territorial_defense` C4 container, SB360 boundary verdicts, and a `NOTICE` entry — but commit 2's **NEGATIVE construct-validity verdict** (below) **DEMOTED it to experimental**. Per the standing rule (*a metric that fails validation must not ship as that metric*), the public metric surface is REMOVED: `compute_territorial_defense` + arms out of `territorial_defense.__all__` (now empty), the 3 columns out of `feature_glossary` (glossary count reverts −3 → 394), and the entry out of the SB360 boundary audit (`BOUNDARY_ENTRY_POINTS`, `NOT_EXERCISED_BUDGET` 50→46); the code is retained in the private `._compute` / `._arms` / `._engine` modules for the replacement-ghost redesign (mirrors the TF-60 Layer-3 arms). The `territorial_defense` C4 container stays (code exists; no analyst-facing edge) and `NOTICE` keeps the attribution for the retained code (Fernández & Bornn, Spearman, Le et al. 2017). Owner-run validation driver `scripts/validate_territorial_defense.py` runs the full spec-§9 probe battery (mirroring `build_tf19_instrument_responsiveness`): per-scored-frame dose-response + paired single-player placebo controls → pooled Layer-0/Layer-1 instrument-validity verdicts, the Arm-B slippage rollup, and the locked pre-registered elite-defender prior (ADR-052 shards, ADR-037 clean-tree, ADR-056 input contract) — **reported-not-gated**. **Commit 2 ran it on the full 321-match SB360 corpus (`docs/research/territorial_defense_construct_validity/`): the NEGATIVE verdict — Layer-0 `instrument_void` + Layer-1 `not_responsive` (pooled dose median 0.0 vs single-player placebo p95 0.137, n_domain 23,366; named-defender face-validity 2/3 at noise-level magnitudes; Arm-B slippage honest-NaN on anonymous SB360). Mirrors the TF-60 Layer-3 / xtgk_v2 outcomes; promotes nothing, triggers no retrain, and drives the demotion above.**
- **Corpus registry (scripts-only, not in the wheel):** the redistributable public SkillCorner set grew **10 → 20** (a second public drop on 2026-09-09, uploaded to pining-for-the-data); `scripts/_corpus.py::PUBLIC_CORPUS` + the `tests/_skillcorner_sample.py` sample loader are updated in lockstep (27 public matches total incl. IDSSE). **The bundled xShot/xCross/ghost models are NOT retrained** — their model cards' "17 matches" correctly records the original public corpus they were trained on; a future public retrain would draw from the larger set.

## [4.111.0] — 2026-09-09

**TF-60 ghost-GK both-axes convention fix + Layer-3 counterfactual engine — deterrent arms demoted to experimental (PR-S182, ADR-089).** The originally-planned Layer-3 deterrent arms **did not survive out-of-sample validation as deterrent metrics**, so this cycle ships the ghost-GK convention correctness fix + re-fit weights, the counterfactual **engine as private experimental infrastructure**, and the **validity study** that motivates a redesign — but **no public deterrent metric**. **Additive to VAEP — no VAEP retrain; C4-free (derived-feature-column count reverts 398→394).** Three commits (code; re-fit weights; validation artifacts + demotion).

- **Ghost-GK goal-relative convention unified to the correct both-axes 180° point reflection (retrain all 5 variants).** `extract_ghost_gk_features` previously flipped only x (`105−x`), leaving signed-y features (`ball_y`, `ball_vy`, `atk_cy`, `ball_to_goal_angle`) and the target `gk_y` in absolute coordinates — not orientation-invariant (one physical scene scored differently at the two goal ends). The extractor now routes every signed-y feature + the target through module-level self-inverse `_to_gr_x`/`_to_gr_y`/`_to_gr_vx`/`_to_gr_vy` (hoisted so the extractor and serve share one function), matching the both-axes convention the ghost-outfield model already used. The chirality gate is strengthened to catch a y-only regression. **Retrain trigger:** all 5 bundled variants are re-fit; the feature-contract + chirality fingerprints change.
- **Both ghost serves return frame-ready `ghost_x`/`ghost_y`** (`serve_ghost_gk_positions`, `serve_ghost_outfield_positions`) via each model's own goal-relative inverse, keeping `ghost_gr_x`/`ghost_gr_y` as audit — no consumer re-derives orientation.
- **gkdv `_engine` consumes the serve's frame coords** (deletes its x-only goal-relative→frame math). Load-bearing: once the ghost-GK model is both-axes, the old x-only engine conversion mislocated the away-team keeper in y. **gkdv arm values change for away-team keepers** (gkdv re-materialize owed; the TF-19 sign-off is re-run from the new weights).
- **Layer-3 counterfactual arms DEMOTED to experimental — no public deterrent metric ships.** An out-of-sample validity study (179-match corpus, 15,113 turnovers; `docs/research/tf60_layer3_construct_validity/`) found the outfield deterrent arms **confounded with attacking commitment**: the outfield-space arm's matched-ATT signal (−0.0035, t −5.2 with no controls) collapses to ≈0 once commitment is controlled (t −0.82), with ~76 % of the effect mediated by turnover position (direct effect t 0.92). Only the keeper-threat arm was control-robust (ATT −0.0013…−0.0023, significant in all four control sets, not mediated) — but pooled/SkillCorner-driven, marginal on Gradient Sports, underpowered on full tracking. Per the standing rule (*a metric that fails its validation must not ship as that metric*), the arm functions + columns are **removed from the public `silly_kicks.restdefense` surface** (`__all__`, `feature_glossary`, the SB360 boundary audit — `NOT_EXERCISED_BUDGET` stays 48, the C4 derived-feature-column count reverts 398→394); the code is retained in the private `restdefense._arms` / `_counterfactual` modules for the redesign. The **metric-free ghost-frame engine** `build_restdefense_ghost_frames(which="keeper"|"rearguard", …)` (a gkdv sibling — PURE; drop-and-counted `RestDefenseGhostReport`; a non-finite served ghost raises) + its report **stay public**. A **within-turnover counterfactual-counter redesign** — which removes the commitment confound the possession-aggregate arm conflates — is registered (`docs/superpowers/specs/2026-09-08-tf60-counterfactual-counter-redesign-design.md`).
- **Five correctness bugs fixed red-first during implementation.** (1) The keeper counterfactual scored `no_ghost_served` on every in-domain frame (the GK serve is keyed `(frame, team)` with no `player_id`; the possession restriction used a Series-vs-Series `ids_match`, which is Series-vs-scalar → always False). (2) `_opponent_by_frame` resolved B to the FIRST team per frame (= A) via the same `ids_match` misuse, so the outfield arm priced A's OWN accessible space, not the opponent's — masked because the arm tests never asserted the value; fixed with `ids_equal`/`ids_differ`. (3) `compute_rest_defense` crashed (`KeyError('rd_num_superiority')`) on a match with zero committed-forward samples — fatal to a corpus pass — now returns an empty declared-columns table. (4) `add_ghost_gk` double-flipped away-team y once the model served both-axes goal-relative `gr_y` (the ADR-028 action-LTR reprojection was written for the x-only model: flip-gated `68−gr_y` → uniform); the VAEP-feature path only, masked by the Phase-A transient-red chirality set. (5) `rest_defense_outfield_deterrent` crashed on a keeperless frame (unresolvable goal → `GoalEndUnresolvedError`) instead of degrading to honest-NaN like its siblings (ADR-055).
- **Mixed-provider identity-column fix in the three validation drivers** (`build_tf60_layer3_arm_values` / `build_gkdv_arm_values` / `build_tf19_instrument_responsiveness`): `game_id` / `team_id` / `keeper_key` are now CANONICAL (string), not raw — raw ids are int (Gradient Sports) vs str (SkillCorner / IDSSE), so a mixed-provider shard set was an unwritable mixed-dtype parquet column (pyarrow `ArrowInvalid` on the combine, which also lost the `run_commit` manifest). Pre-existing gkdv/tf19 defect surfaced by the 179-corpus validation (ADR-019).
- **The pining SkillCorner loader reads the new artifact tracking schema (rode this cycle to run the full-corpus validation).** Newer SkillCorner artifacts ship tracking as a PARQUET table (`_tracking.parquet`, nested `ball_data`/`player_data`) instead of `_tracking_extrapolated.jsonl`; `build_skillcorner_frames` opened it as UTF-8 text and crashed, so the DGX validation drivers could not load any SkillCorner match. The re-fit *weights* are unaffected (training reads the tc3 cache, never the raw loader).
- **Validity study shipped** (`docs/research/tf60_layer3_construct_validity/`): the convergent-anchor + matched-ATT + mediation harness (`harness/`), the findings JSONs, and `findings.md` — deriving from the single clean arm table (`run_commit a867f9b`).
...
- **Leakage-safe 20/16-feature vector.** No feature encodes the modeled team's own rearguard coordinates (the prediction target); the vector is ball state + opponent counter-threat geometry + context + the lateral `slot_index` rank. Enforced by an output-invariance guard (perturbing the target leaves features byte-identical).
- **Possession-conditioned — ONE model, both regimes (ADR-055 idiom).** Training extracts BOTH non-ball teams' deepest-`n` per frame with a live `team_in_possession` discriminator (1 = the ball-carrier's rest-defense rearguard, 0 = the defending line facing the attack); serving defaults to the in-possession slice. Measured separable on real data: in-possession CV MAE 6.97 m vs out-of-possession 5.04 m from one model.
- **Standard ghost-artifact conventions, reused verbatim.** Pickle-free `model.npz` + `metadata.json` + `SHA256SUMS`; the ghost-GK numba leaf-walk (ADR-076); fail-closed `load()` (SHA-256 + chirality + feature-contract, ADR-011/040/050) with a **pandas-major-invariant** chirality frame hash (the probe frame is coerced to native scalars before hashing, so a bundle loads identically under pandas 2 and 3 — CI spans both, ADR-057; the bundled-weights load+finite-predict CI gate runs on every leg); velocity-keyed `default`/`position_only` variant auto-selection (ADR-067; a velocity-less freeze-frame with no bundled `position_only` → honest-NaN, never the invalid default); FOV cropped-honest-NaN serving (ADR-077; a served frame whose rearguard region is under-observed is `ghost_outfield_source="fov_cropped"` NaN, never a fabricated ghost from the deepest-`n` VISIBLE players). Source token `GHOST_OUTFIELD_SOURCE_VALUES = {"computed", "variant_unavailable", "fov_cropped"}`. The serve resolver treats an ABSENT variant as honest-NaN but **RAISES** on a PRESENT-but-unloadable artifact (a bad SHA / chirality skew is corruption, never a silent all-NaN column).
- **Trained at 1 frame/second** (mirroring the ghost-GK trainer) — 25 fps tracking is massively autocorrelated, so 1 fps is ~25× fewer rows with no meaningful signal loss for a mean-positioning model; folded into the shard-generation token (the 4.77.1 stale-shard rule). Bundled from the **179-match public corpus** (64 GS + 108 SkillCorner + 7 IDSSE), `training_commit=b68328a` (clean tree), sklearn 1.9.0, 4.17M rows. Held-out CV euclidean MAE **6.00 m** (`default`) / **6.07 m** (`position_only`); per-slot 5.96–6.11 m; rearguard coherence `ordering_fraction=1.0`.
- **Trainer** `scripts/train_ghost_outfield.py` (ADR-052 `for_each` sharding; ADR-063 two-commit provenance via `require_clean_tree`; StratifiedGroupKFold `game_id`×`source_provider` CV reporting per-slot / per-provider / per-possession MAE) + publisher `scripts/publish_ghost_outfield.py`. The trainer resolves the tc3 cache's flat `_actions/{game}.parquet` and reads the `meta.json` home for the `score_diff` perspective, and subsamples to `--subsample-fps` (default 1.0).

## [4.108.0] — 2026-09-04

**TF-54 + TF-55 — player-quality bundle: territorial dominance + Glicko-2 duel ratings (PR-S179, ADR-086).** Two sibling **event-only** `compute_*` packages on the shared per-`(player, match)` grain, shipped as one cycle (owner-chosen "Bundle A", grouped by grain). Both mirror the `restdefense/` / `shot_stopping/` idiom (frozen `Params` + `_columns` schema + `_report` sidecar; a `compute_*`, NOT an `add_*`), import `spadl` + `id_compat` ONLY (never `tracking`; AST import-allowlist gate, nothing imports them), group on the CANONICAL player id and emit the RAW id (ADR-019), drop-and-count honestly (ADR-042), document every metric column in `feature_glossary` + `NOTICE`, and add a C4 container each. **Additive — no VAEP / tracking retrain, no re-materialize, C4 aggregator count unchanged (33).** Validated end-to-end on real StatsBomb WC2022 open data.

- **TF-54 territorial dominance (`silly_kicks.territory`).** `compute_territorial_dominance(actions, *, xt, method="completed_failed", window=None, params) -> (samples, TerritoryReport)`. A defender's *territory* is the **trimmed convex hull** (`trim_fraction=0.70`) of their own-half defensive-action locations (tackle / interception / clearance). Opponent passes whose **destination** lands inside the hull are valued by an **INJECTED** fitted `ExpectedThreat` at that destination (`values_at_points` — the xT VALUE, not `rate`): **conceded** (completed) vs **prevented** (failed), plus forward-only variants, per-pass rates, and hull area / centroid context (12 metric columns). Valuation is a **pluggable `method=` family** — `completed_failed` (default) with a reserved typed `counterfactual` door (mirrors `ExpectedThreat(method=…)`). Frame reconciliation is library-owned: the hull is in the defender's action-LTR frame and opponent passes are point-reflected `(105−x, 68−y)` into it for membership (ADR-028). A degenerate hull (< 3 non-collinear actions) → a `territory_hull_source="degenerate"` NaN-metric row, counted; the `TerritoryReport` conserves `n_scored + n_degenerate_hull + n_no_actions == n_players_in`. **Known under-count:** a failed pass's SPADL `end` is the death location, not the intended target, so a pass aimed into the territory but dying outside the hull is not counted.
- **TF-55 Glicko-2 duel ratings (`silly_kicks.duels`).** `compute_duel_ratings(actions, *, initial_ratings=None, window=None, window_stat="as_of_end", params) -> (samples, DuelRatingReport)`, one row per `(game_id, player_id)`: `duel_rating` / `duel_rating_deviation` / `duel_volatility` + `duels_contested` / `_won` / `_lost` + a `duel_winner_source` provenance (`native` / `derived`). **Glicko-2** (Glickman: SCALE=173.7178, Illinois volatility update, inactivity RD growth) with the **match as the rating period** — matches process in ascending-`game_id` order (a chronology proxy) with ratings carried forward. **A rating system is long-lived: `initial_ratings` RESUMES a later batch and `DuelRatingReport.final_ratings` (canonical-keyed) is the resume seed** — `compute(m1+m2)` final state equals two batches threaded via `initial_ratings` (byte-equal; resume-equivalence gated on real data). **`window`** (a Collection of `game_id`s) slices the trajectory (one row/player, `game_id` NA; ratings cumulative NOT summed): `window_stat="as_of_end"` (default) or `"change"` (delta from before the window). Duels are **native** (sportec `tackle_winner/loser`) or, absent that, **derived** from the `tackle` / `take_on` result adjacency (an adjacent cross-team pair within 5 s; winner = whoever's action succeeded); the strategy is chosen at frame-set granularity, never per-duel-guess. **Ground duels only** (SPADL has no aerial type); an **indeterminate** duel (no clear winner) is **EXCLUDED and counted** (`DuelExtractReport`), never a fabricated winner. A pure, leakage-free **`update_glicko`** primitive (validated against Glickman's published worked example) backs the orchestrator; `extract_duels` is public. **Known limit:** the derived path only pairs adjacent `tackle`/`take_on` (~11 duels/match on StatsBomb WC2022) — a richer adjacency is a follow-on. Both winner paths are e2e-verified on real data — native on the committed IDSSE/sportec slice (regular suite), derived on StatsBomb WC2022 (`@e2e`).
- **Per-provider params ship EMPTY** (ADR-009): `TerritoryParams.for_provider` / `DuelRatingParams.for_provider` resolve to the base config until a separate gated apply-PR. Attribution: Sumpter, Soccermatics / Twelve.football 'Earpiece' (module 10.2) for TF-54; Glickman, 'The Glicko-2 rating system' for TF-55.

## [4.107.0] — 2026-09-02

**TF-59 PR2 — GK shot-stopping metric: Goals Prevented / GSAA (PR-S178, ADR-085).** A new event-only `silly_kicks.shot_stopping` package computing **Goals Prevented == GSAA** per `(goalkeeper, match)` from an INJECTED per-shot Post-Shot xG + the resolved defending keeper (PR1). **Additive — no VAEP / tracking retrain, no re-materialize, C4 aggregator count unchanged (33).**

- **`compute_shot_stopping(actions, *, psxg_column, defending_gk_column="defending_gk_player_id", defending_team_column="defending_gk_team_id") -> (samples, ShotStoppingReport)`** — one row per `(game_id, keeper)`: `shots_faced` / `goals_conceded` / `psxg_faced` / `goals_prevented` (== GSAA = ΣPSxG − GA), each with an `_excl_penalties` companion. PSxG is INJECTED (port pattern; silly-kicks ships no xG/PSxG); PSxG-presence IS the on-target gate. Own goals (`bad_touch`, ADR-018), blocked shots (ADR-046), and the penalty shootout (period 5) are excluded; penalties reported both ways. Keepers are grouped on their CANONICAL id (`canonical_id_series`, ADR-019) — raw id emitted via `.first()` — so a mixed-dtype keeper id is not fragmented into two split-GSAA rows (byte-identical for a single-dtype provider). Output ids are RAW.
- **Honest attribution census** — a shot with no resolved defending keeper is excluded from per-keeper rows but COUNTED in the conserving `ShotStoppingReport` (ADR-042), never dropped-silent nor misattributed.
- **`keeper_identity.add_defending_gk_player_id` now emits the authoritative `defending_gk_team_id`** (the opponent team, read from the `keeper_map` VALUE) on BOTH paths — **amends ADR-084's byte-identity-when-omitted contract** (the omit path gains one additive column; existing column VALUES unchanged; `add_defending_gk_player_id` is new in 4.106.0, so exposure is minimal). **`KeeperIdentity` gains a raw `team_id` field** (gold-standard, ADR-085; additive + defaulted `pd.NA`, populated by `resolve_keeper_identities` on both the roster and native paths — `KeeperIdentity._fields` changes, so exact-arity 3-tuple unpacking would break, none in-tree), so the `keeper_map` is self-sufficient: the defending team resolves even for an opponent that never appears in the actions (a frame-seeded map). Output ids are RAW, consistent with `defending_gk_player_id`.
- **Event-only:** `shot_stopping` imports `spadl` (config) + `id_compat` + `keeper_identity` ONLY, never `tracking` (AST import-allowlist gate). The 8 metric columns are documented in `feature_glossary` (a 6th `compute_*` leg) + `NOTICE`; a `shot_stopping` C4 container is added + `architecture.html` re-rendered via Graphviz `dot`.

## [4.106.0] — 2026-09-02

**TF-59 PR1 — keeper-appearance resolver + four provider extractors (PR-S177, ADR-084).** Prerequisite infrastructure for the TF-59 GK shot-stopping metric (Goals Prevented / GSAA), which ships in PR2 — this PR ships nothing consumer-facing on its own. Promotes the keeper-identity resolver OUT of `tracking/` to the public top-level `silly_kicks.keeper_identity`, adds an **event-only** enumeration path, and **un-forecloses substitution / red-card keeper resolution** (amending ADR-078) via an injected appearance-interval port. **Additive — no VAEP / tracking retrain, no re-materialize, C4 count unchanged (33).**

- **Breaking import-path move, NO shim** (ADR-019 / ADR-043 clean-break precedent): `silly_kicks.tracking.resolve_keeper_identities` / `KeeperIdentity*` / `add_defending_gk_player_id` / `apply_keeper_identities_to_frames` now live in the public top-level module **`silly_kicks.keeper_identity`**. A stale external pin fails loud at import (never silently). The `native` identity path lazy-delegates to `tracking._gk_resolve`, so `import silly_kicks.keeper_identity` adds no `tracking` import edge. In-repo callers (`tracking/{features,_run_features,__init__}.py`, `scripts/{build_tf19_instrument_responsiveness,_sb_roster,_sb_battery}.py`, `docs/PRIVATE_CONSUMERS.md`) migrated.
- **Event-only path:** `resolve_keeper_identities(actions, frames=None, *, identity, roster=None)` resolves keeper identity from SPADL actions alone — the shot-stopping metric needs no tracking frames.
- **Injected `KeeperAppearances` port:** `validate_keeper_appearances` / `KEEPER_APPEARANCE_COLUMNS` — one row per `(keeper, period)`, keyed per `(game, period_id, team)`, PERIOD-RELATIVE times (ADR-017), `object` ids (string-tolerant; DFL / SkillCorner ids are strings) — plus the shared per-period builder `build_keeper_appearances_from_segments(segments, periods, *, game_id)` (`KeeperSegment`) that decomposes each keeper's on-pitch segment into one row per period it spans. Source vocabulary `{native_intervals, sub_events, starting_xi, emergency_keeper}`.
- **Interval-granular attribution:** `add_defending_gk_player_id(actions, keeper_map, *, appearances=None)` — when an appearance table is supplied, each action's defending keeper is the opponent keeper whose `[start, end)` interval covers the action's `time_seconds`, so attribution flips exactly at the substitution minute; a `defending_gk_source` provenance column records the appearance-vs-map cross-check over `{appearance, map_fallback, appearance_map_conflict, unresolved}`. **Byte-identical, and with no extra column, when `appearances` is omitted.**
- **Four provider extractors** (`providers/<p>/appearances.py` — StatsBomb / Sportec / Gradient Sports / SkillCorner): each parses its native appearance/substitution encoding (StatsBomb `Starting XI` + `Substitution` events; Sportec/DFL `PlayingPosition="TW"` subs + `PlayerBecomesGoalkeeper` emergency keepers; Gradient Sports roster + `gameEventType=="SUB"`; SkillCorner `players[].playing_time.by_period[]` intervals) into the normalized port via the shared builder, depending on the port + `id_compat` + pandas/stdlib ONLY. Enforced by an AST module-level import-allowlist gate (`tests/providers/test_appearances_import_allowlist.py`) — a fifth `appearances.py` fails the completeness meta-assertion until added deliberately.
- **ADR-084 amends ADR-078** (un-forecloses substitution-event keeper resolution). Starter identification stays irreducibly provider-specific (per-provider maintenance).

## [4.105.0] — 2026-09-01

**TF-60 PR3 — rest-defense GK-ghost re-fit: extended-grid `sweeper` variant pair (PR-S176, ADR-083).** The TF-60 §9 in-possession-validity gate **fired**: the shipped `GhostGkModel` hard-saturates at `GRID_X_MAX = 30 m` (training drops `gk_x > 30 m` labels as "sweeper rushes"), so it cannot place an in-possession high sweeper (30–45 m off its own goal) — and `ghost_out_of_box` is blind, because the clip is applied to the output while out-of-domain is an *input* property. This ships an **additive** extended-grid `GhostGkModel` variant pair, `sweeper` (faithful, 26 feats) + `sweeper_position_only` (velocity-less SB360, 21 feats), so the Layer-3 GK arms (PR4) can consume `from_variant("sweeper")` — never the frozen default. Finding + reproducible probes: `docs/research/tf60_ghost_gk_in_possession_validity/`.

- **The ghost-GK grid is now a first-class per-model `GhostGridSpec`** (was module-global `GRID_*` constants), threaded through `__init__` / `save` / `load` / the label-domain filter / `ghost_out_of_box`. **Byte-identical for the frozen `default` / `position_only` / `full`** — a full metadata-SHA round-trip is *unachievable* (`save()` recomputes `feature_contract.probe_sha256`), so byte-identity is proven THREE ways instead: the pre-existing golden / chirality / feature-contract / KDE-density tests pass unchanged, `DEFAULT_GHOST_GRID.to_metadata_dict()` equals `default`'s committed `grid_spec` field, and a re-save of `default` differs only in `feature_contract` ([[reference_trained_model_metadata_sha_never_roundtrips]]-class).
- **`GhostGkVariant` gains `sweeper` / `sweeper_position_only`; the resolver branch is ADDITIVE**, placed before the `model is not None` custom short-circuit, and velocity-keys within the sweeper family (faithful ↔ position_only). A missing `sweeper_position_only` returns `(None, "sweeper_position_only")` — the honest NaN degrade, NEVER the invalid-on-velocity-less default (ADR-067 asymmetry). **GKDV passes `None` / `"default"` / an instance and can NEVER select `sweeper` → NO GKDV retrain, NO VAEP retrain.**
- **`predict_density` fail-loud on a non-default grid** (the KDE grid stays the default 30 m grid). `compute_ghost_gk` / `add_ghost_gk` / `serve_ghost_gk_positions` use the grid-independent `predict_mean` path and WORK on the extended grid.
- **Trainer / publisher:** `train_ghost_gk.py` gains `--grid-x-max` (keyed into the shard-generation token — the 4.77.1 stale-shard rule: the lifted ceiling changes the retained label ROWS) + `--variant sweeper|sweeper_position_only` + `>30 m`-stratum MAE and per-provider `>30 m` coverage in `metrics.json`; `publish_ghost_gk.py` builds its sanity sample from the metadata `feature_names` (fixes a latent position_only 21-vs-26 count bug).
- **Bundled artifacts** (`_ghost_gk_weights/{sweeper,sweeper_position_only}/`, trained on the same 179-match public corpus as `default`, from a CLEAN `training_commit`): CV MAE euclid **1.142 m** (faithful) / **1.164 m** (position_only), boosted-reconstruction parity **1.2e-13** (exact — safe to publish), all acceptance criteria PASS. §9 payoff (REPORTED, not gated, per spec §6/§7.3): `>30 m`-stratum MAE **2.06 m** / **2.03 m** — the sweeper *places* high keepers where the default is blind. `>30 m` coverage is **IDSSE/Sportec-dominated (11.5 %)**; SkillCorner 0.24 %; **Gradient Sports 0.0 %** — investigated and found to be a **Gradient Sports source-data limitation** (below), not a sweeper defect.
- **Gradient Sports goalkeeper-position clamp (found + guarded).** GS clamps the goalkeeper's tracked position to a hard **27.5 m from goal** — universal and exact across matches/keepers on the raw provider data, keeper-specific (outfielders roam the full pitch) — which silly-kicks faithfully passes through (native keeper identity, no coordinate clamp). So any GS goalkeeper-depth analysis is invalid past 27.5 m. New public diagnostic `validate_gk_position_clamp` / `GkClampDiagnosis` / `GoalkeeperClampWarning` (fifth member of the `validate_time_base` / `validate_velocity_regime` / `validate_fov` family) detects the clamp signature (a hard ceiling with a pileup) provider-agnostically, and `tracking.gradientsports.convert_to_frames` emits the warning automatically. Additive; no retrain. Evidence: `docs/research/gs_keeper_clamp/findings.md`.
- **Additive — no GKDV/VAEP retrain, no re-materialize, C4 unchanged** (no new feature columns — the variant serves the existing `ghost_gk_x/y`).

## [4.104.0] — 2026-08-31

**TF-19 A+2 — physics-arm instrument-validity (Layer 0) + responsiveness (Layer 1) probes + named-keeper validation (PR-S175, ADR-082).** Gives the shipped GKDV physics arms (`delta_das`, `delta_threat_suppression`) a demonstrable, **reported-not-gated** instrument-validity + named-keeper check on the WC2022 Gradient Sports corpus. **Additive — no gate flip, no VAEP retrain, no re-materialize, C4-free.**

- **New `silly_kicks/gkdv/_probe.py`:** the dose imposer `impose_defending_keeper_dose` + the POOLED-corpus verdicts `layer0_instrument_verdict` (instrument validity; `arm_unscoreable` is a first-class verdict DISTINCT from `instrument_void`; the `>= SATURATING_MULTIPLE` (5×) realistic leg requires `real_med > 0` so `5*0` never vacuously validates a dead zero-dominated ΔDAS instrument — the placebo leg is the backstop) and `layer1_responsiveness_verdict` (`gk_med >= PHYSICS_ARM_PROBE_RATIO` (2.0) `* max(nd_med, placebo_p95)`, a NEW registration) + `paired_vector_controls` (parent `_model_eval` idiom: ONE defending outfielder per control — nearest alone + R single-outfielder placebos — so `nd_med`/`placebo_p95` are DISTINCT). Verdicts are POOLED-corpus statistics computed in a REDUCE over ALL shards, never per shard.
- **Arm-key seam** `gkdv._validate.expected_direction_for_arm` (arm output column `delta_threat_suppression` → direction key `delta_threat`; an unmapped arm RAISES).
- **Owner-run driver `scripts/build_tf19_instrument_responsiveness.py`** (ADR-052 `for_each` shards + pooled reduce; ADR-037 clean-tree provenance; ADR-056 input-contract): resolves keeper identity DRIVER-side via `tracking.resolve_keeper_identities` (ADR-078), emits the §6.1 `gate_eligible` census + a §6.2 per-keeper sign table checked against a PRE-REGISTERED `NAMED_KEEPER_PRIOR` (Alisson/Neuer → deterrent, LOCKED before the run) + Layer-4 anchoring.
- **Finding (WC2022 GS, 64 matches / 123,430 domain frames; reported-not-gated):** ΔDAS is a **weak keeper-deterrence instrument** — Layer 0 `instrument_void`, Layer 1 `not_responsive` (accessible space is outfield-dominated, so the one deep keeper barely perturbs it: realistic median |ΔDAS| 0.41, saturating-goal-line 0.12, placebo p95 2.94). Named-keeper prior **1/2 confirmed**: Alisson −0.55 (deterrent, meets prior), Neuer +0.02 (≈zero, does not); 15/32 gate-eligible keepers match the expected-negative sign; anchoring `uninterpretable`. The threat arm is `arm_unscoreable` (needs a fitted `ExpectedThreat` the package does not ship). Full artifact in `docs/research/tf19_instrument_responsiveness/`.
- **Correctness fix folded in:** `gkdv._probe._build_dose_targets` now deduplicates keeper rows (mirroring `_engine._build_provenance`), so a keeper-substitution window (two defending keepers in one frame — real GS 22/23/24-player frames) no longer fans out the write-back merge; **byte-identical on single-keeper frames**. Regression test `tests/gkdv/test_probe_dose.py::test_multi_keeper_frame_does_not_fan_out`.
