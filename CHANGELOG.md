...
- **HONEST LIMIT — validated as an INSTRUMENT, NOT player-attributable.** The marginal-removal delta is **team-conditioned by construction** (vacated space re-partitions to teammates); the validation corpus (WC2022 + 30 licensed SB360, national-team / single-tournament) has **zero cross-team defender observations**, so per-defender numbers are **NOT a defender ranking** and the elite-defender ("Van Dijk") prior is elite-defender/elite-team **collinear** (FACE-VALIDITY, not attribution). Ranking is a future ADR-009 gated on a crossed defender+team ICC over a multi-club transfer corpus.
...
- **Kept seams landed as reusable infrastructure** (carried from the unmerged `ab9001c`; the event-only *cone* was NOT carried): `expected_passing.PassCompletionModel` (event-only logistic; pickle-free JSON+SHA256; fail-closed load), `xthreat.destination_profiles` + `xthreat/_counterfactual_seam.py`, `scripts/train_pass_completion.py`, `scripts/_sb_open_data.py`. **`PassCompletionModel.bundled()` raises `FileNotFoundError` until the owner-run public-corpus weights land (commit 2);** two `bundled()` tests skip until then.
- **Governance:** +3 `feature_glossary` columns (each `definition` carries the not-a-ranking honest-limit); a `NOTICE` "Territorial defense (SB360 counterfactual)" entry (Fernández & Bornn, Spearman, Le et al. 2017); a `territorial_defense` C4 container; SB360 boundary verdicts (threat arm `differs_by_design` Tier-1 lift; keeperless `gk_absent` now **scores** — GK-independent per-action resolution, ADR-091 — not honest-NaN). Owner-run validation driver `scripts/validate_territorial_defense.py` runs the full spec-§9 probe battery (mirroring `build_tf19_instrument_responsiveness`): per-scored-frame dose-response + paired single-player placebo controls → pooled Layer-0/Layer-1 instrument-validity verdicts, the Arm-B slippage rollup, and the locked pre-registered elite-defender prior (ADR-052 shards, ADR-037 clean-tree, ADR-056 input contract) — **reported-not-gated**.
