...
- **gkdv `_engine` consumes the serve's frame coords** (deletes its x-only goal-relative→frame math). Load-bearing: once the ghost-GK model is both-axes, the old x-only engine conversion mislocated the away-team keeper in y. **gkdv arm values change for away-team keepers** (gkdv re-materialize owed; the TF-19 sign-off is re-run from the new weights, Phase B).
- **Complete Layer-3 arms — `silly_kicks/restdefense/` (a gkdv sibling).** `build_restdefense_ghost_frames(which="keeper"|"rearguard", …)` ghosts the in-possession team A's OWN rearguard/keeper near A's OWN goal when A is committed-forward (PURE; drop-and-counted conservation; a non-finite served ghost raises). `rest_defense_outfield_deterrent` / `rest_defense_gk_deterrent` price the opponent B's counter-danger via gkdv's generic `delta_threat_suppression_batch` / `delta_das_batch` seams (attacker-value units, **negative = deterrent**); `merge_rest_defense` left-joins the arm tables onto the Layer-1/2 samples. A `_probe.py` instrument-validity probe (dose imposers + paired-vector controls) reuses gkdv's PUBLIC verdict functions (`layer0_instrument_verdict` / `layer1_responsiveness_verdict`) with no private-constant import. The keeper arm uses the **sweeper** ghost-GK variant (ADR-083). Corpus driver `scripts/build_tf60_layer3_arm_values.py` (ADR-052 `for_each` + ADR-037 provenance; xT fit once on the loaded corpus and injected — the `measure_cover_shadow_argmax_agreement.py` convention).
- **The outfield arm's keeper-agnosticism is STRUCTURAL isolation, not value invariance (spec §6.2 correction).** The ghost never repositions A's keeper (exact), but the arm value depends nonlinearly on the fixed keeper via `lambda_gk` pitch control (measured: moving A's keeper shifts the threat arm ~3.6%; the DAS/space arm is keeper-blind-generic, ~1%). The original "the keeper's contribution cancels — a property, not a coincidence" is an over-claim, corrected in code + tests.
- **Three correctness bugs fixed red-first during implementation.** (1) The keeper counterfactual scored `no_ghost_served` on every in-domain frame (the GK serve is keyed `(frame, team)` with no `player_id`; the possession restriction used a Series-vs-Series `ids_match`, which is Series-vs-scalar → always False). (2) `_opponent_by_frame` resolved B to the FIRST team per frame (= A) via the same `ids_match` misuse, so the outfield arm priced A's OWN accessible space, not the opponent's — masked because the arm tests never asserted the value. Both fixed with `ids_equal`/`ids_differ` (column-vs-column). (3) `compute_rest_defense` crashed (`KeyError('rd_num_superiority')`) on a match with zero committed-forward samples — fatal to a corpus pass — now returns an empty declared-columns table.
- **The 4 restdefense arm columns are documented in `feature_glossary` (with an emitted-columns leg) and carry a spec-§10 liveness gate** (non-NaN + non-constant on the fixture's `computed` rows; both arms measured live); dtype-safe via `id_compat` (the id-scalar completeness registry is N/A — its population is `spadl`/`atomic`/`vaep`/`causal`/`tracking`, and restdefense, like gkdv, is not in it). **Phase-A transient-red:** the bundled ghost-GK tests (golden / chirality / feature-contract / any `model=None` bundled load) are red on load between Commit 1 and Commit 2; the branch is green at its tip once the re-fit weights are bundled (the `--merge` gate).
