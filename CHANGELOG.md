...
### Added — TF-63 xImpact: in-game win-probability model + `VAEP.rate_ximpact` (PR-S193, ADR-100)

Match-context-weighted action value (Paul, Klemp & Memmert 2025): `xImpact = VAEP_adjusted × ΔP(win | goal at the pre-action game state)` — a late equalizer scores high, a garbage-time goal ≈ 0. Additive, event-only — **no VAEP/tracking retrain, no re-materialize.** `compute_*`, not `add_*` — the action-coupled aggregator count stays 33; +1 C4 container. Two-phase clean-tree provenance: this commit ships the code; the bundled public-corpus weights + the construct-validity report land in the release's 2nd commit (`training_commit` on the clean tree, the TF-53/TF-57/TF-61 2-phase pattern).
...
- `win_prob_leverage` glossaried (`feature_glossary`, 455→456); `ximpact` is a VAEP rating-method output (glossary-exempt like `vaep_value`). `p_win`/`p_draw`/`p_loss` base-name-shared with `match_outcome` — glossary definitions generalized to describe both producers. `win_probability` is `metric_contracts`-exempt (per-action feature-grain, not a per-(entity, match) mart family). `NOTICE` attribution; `WinProbabilityParams` frozen (`for_provider` empty, ADR-009). Decision: ADR-100.
