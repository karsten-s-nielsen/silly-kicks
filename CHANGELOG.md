...
- **Owner-run construct-validity battery** `scripts/validate_gk_decision.py` (ADR-052 shards, ADR-037 clean-tree, ADR-056 input contract) — **RAN on the DGX** over the owner-tier corpus (aggregate-only, reversibility-not-provenance; report `docs/research/gk_decision_construct_validity/`). **Native (full 906-match corpus / 22,934 decisions / 153 keepers): VALIDATED as an instrument** — responsive (`decision_pct` t≈+27), discriminating (keeper ICC 0.03–0.04 vs a permutation null ~0.001, p=0), net-of-team real (club-adjusted ICC 0.07, p=0), transfer inconclusive (11 usable crossing keepers) → **ranking not licensed**. **Reconstruction fidelity** (SkillCorner Rosetta Stone, per `(keeper, game_id)`) at the shipped default: sel-eff ρ ≈ **0.30**, decision_value ρ ≈ **0.26** (both p<0.001) — moderate, significant. **The reachability sweep is a threshold GRID**: SB360 INVERTS at reachability 0.5 (the bundled WC2022 xPass runs generous on SB360 — 95% of reconstructed options ≥0.5, so a 0.5 filter can't bite) and is responsive at ~0.85, so **the shipped default is 0.85** — 0.5 was a useless default (inert on the native tier, inverting on SB360). A deeper per-provider xPass recalibration for SB360 remains a future **ADR-009** refinement (NOT required for responsiveness — the 0.85 default handles it). **Reported-not-gated, promotes NO default.**
