...
Corpus (REVIVAL delta): the battery runs over the FULL PUBLIC StatsBomb open-data corpus
(``all_open_competitions()``) by default, not a single tournament -- a single competition is far too
thin for a team-discrimination / split-half read. ``--competitions-json`` narrows to a chosen list of
``[[competition_id, season_id], ...]`` (mirroring ``train_match_outcome_dependence.py``);
``assert_statsbomb_open_data_mode`` fail-closed-guards that the manifest is the redistributable open
one before any load. The ``for_each`` generation carries a ``"source": "open-data-all"`` corpus marker
in its token so a broad-corpus generation cannot collide with a single-competition one (the 4.77.1
stale-shard rule).
...
Usage (owner; scripts/ on sys.path):
  python scripts/validate_territory_counterfactual.py --out <DIR> [--competitions-json FILE] \
      [--max-per-provider N] [--n-folds K] [--match-ids-json FILE]
"""
...
#: The locked elite-defender ("Van Dijk") prior, RE-DRAWN for the FULL PUBLIC open-data corpus
#: (REVIVAL delta): acknowledged elite centre-backs matched by an ASCII-folded surname substring
#: against the roster ``player_name`` (so an accented roster name like "Gerard Pique" still matches).
#: KEEPS the branch's national-team CBs (present across WC2018/WC2022, both in the open manifest) and
#: ADDS one unambiguously-elite club CB (Pique) reachable in the broad open corpus (La Liga is heavily
#: covered); common-surname CBs ("Ramos"/"Dias") were dropped -- their ASCII-fold substring can hit >1
#: player (owner ruling 2026-09-17), diluting the face-validity signal.
#: A non-empty frozen constant; str members per SPEC-04. The validator's own ``_elite_name_match_counts``
#: census surfaces any name that resolves to 0 (absent) or >1 (ambiguous) players, so a corpus miss is
#: visible in the artifact, never silently wrong. LOCKED PRE-REGISTRATION -- owner-confirmed at commit.
ELITE_DEFENDER_PRIOR: frozenset[int | str] = frozenset(
    {
        # National-team CBs carried from the branch (present in WC2018/WC2022, both in the open manifest).
        ...
        # Club CB added for the broad open corpus (La Liga is heavily covered). Kept to ONE
        # distinctive-surname club CB: "Ramos"/"Dias" were dropped as common surnames whose ASCII-fold
        # substring can resolve to >1 player (owner ruling 2026-09-17), which would dilute the prior.
        "Pique",  # Gerard Pique (Barcelona / Spain) -- La Liga open data
    }
)
...
PRE_REGISTRATION_LOCKED = "2026-09-17 (broad-corpus revival; before the owner run)"
...
def main() -> None:
    ...
    ap.add_argument(
        "--competitions-json",
        default=None,
        help="JSON [[competition_id, season_id], ...] narrowing the corpus. Default: all_open_competitions() "
        "(the FULL public StatsBomb open-data manifest -- a single tournament is too thin for the "
        "team-discrimination / split-half read).",
    )
    ap.add_argument("--max-per-provider", type=int, default=None, help="cap the GLOBAL number of matches")
    ...
    # The FULL public open-data corpus by default (revival delta); --competitions-json narrows it. The
    # locked elite-defender prior is re-drawn for this broad corpus; the name-match census surfaces any
    # miss. Both paths yield the same 5-tuple, so for_each consumes them unchanged.
    competitions = (
    ...
    corpus_label = (
        ...
        if args.competitions_json
        else "statsbomb-open (all_open_competitions)"
    )
    ...
    res = for_each(
        ...
        token_inputs={
            ...
            # Corpus marker (the 4.77.1 stale-shard rule): a broad-corpus generation must not collide
            # with a single-competition one. "open-data-all" is the default (full manifest); a narrowed
            # --competitions-json run keys on its explicit competition list so two narrow runs disjoin.
            "source": "open-data-all"
