...
evaluates the common-public-held-out PAIRED data-effect comparison (spec S5, P1), computes
FAIL-CLOSED acceptance gates (spec S3, N3), and writes a pickle-free artifact ONLY if the
gates pass. Quality numbers in metrics.json are CV/protocol estimates, not the shipped
all-data fit (N7).
...
def _extract(source, horizon_seconds) -> tuple[pd.DataFrame, np.ndarray, np.ndarray, np.ndarray]:
...
def _paired_data_effect(X, y, groups, is_public, *, shared_params, negative_subsample=None, seed=42) -> dict:
    """Common-public-held-out PAIRED data-effect test at FIXED shared hyperparameters (S5, P1).

    For each public outer fold k: fit `public` on public-train and `full` on public-train + ALL
    GS rows, BOTH at `shared_params` (only training data differs; full never sees public-test
    games). Score both on the public held-out fold. Δ_k = PR-AUC(full) - PR-AUC(public).
    Ship-two iff Δ_k > 0 in >= K-1 of K folds AND mean Δ > 0. ``negative_subsample`` thins
    negatives in each arm's TRAIN data only (eval fold keeps the true balance).
    """
    import xgboost as xgb
