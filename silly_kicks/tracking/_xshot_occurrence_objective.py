...
def _cv_logloss(X: pd.DataFrame, y: np.ndarray, groups: np.ndarray, params: dict) -> tuple[float, float, float]:
    """Match-stratified GroupKFold CV -> (mean log-loss, mean PR-AUC, mean Brier)."""
    ...
    for tr, te in gkf.split(X, y, groups):
        ...
        clf.fit(X.iloc[tr].to_numpy(dtype=float), y[tr])
