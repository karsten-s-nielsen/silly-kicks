...
def _resolve_per_period_flips_metrica(
    *,
    ...
    events-side and Sportec events-side API exactly.
    """
    ...
    if "period" in events.columns:
        direction.require_et_direction(
            events["period"], home_team_start_left_extratime, source="metrica convert_to_actions"
        )
...
def convert_to_actions(
    ...
    *,
    ...
    # time-neighbour lookup resolves the true neighbour. _build_raw_actions is
    # order-preserving (removal/1:1/adjacent-synth-insertion, no reorder), so this
    # top sort is the only ordering seam. Empty frame passes through unchanged.
    # See sort_actions_chronologically in .base.
