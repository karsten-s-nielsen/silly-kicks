...
- **Tracking** (`tracking/`): Per-frame namespace parallel to `spadl/`. 19-column long-form schema (`TRACKING_FRAMES_COLUMNS`); native Sportec + PFF adapters; kloppy gateway for Metrica + SkillCorner. Linkage primitive (`link_actions_to_frames` + `slice_around_event`) joins SPADL actions to frames; pointer-DataFrame return + `LinkReport` audit (ADR-004). Tracking-aware features ship in `tracking.features`: PR-S20's 4 `action_context` features + `add_action_context` aggregator + `tracking_default_xfns`; PR-S21's 4 `pre_shot_gk_*` features + `add_pre_shot_gk_position` aggregator + `pre_shot_gk_default_xfns`. Events-side `silly_kicks.spadl.utils.add_pre_shot_gk_context(*, frames=None)` lazy-imports the GK aggregator when frames are supplied (per ADR-005 § 5). Mirror atomic-SPADL surface in `atomic.tracking.features`; VAEP integration via composition (`compute_features(*, frames=None)` + `_frame_aware` xfn marker dispatch). Decision: ADR-005. Pressure / pitch control / smoothing / multi-frame interpolation tracked as TF-2..TF-9 in TODO.md On-Deck; GK angle / frame-based GK ID / defensive-line features tracked as TF-12..TF-14.
