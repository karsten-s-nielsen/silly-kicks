...
def test_territorial_dominance_counterfactual_runs_on_statsbomb_open():
    ...
    if not matches:  # type: ignore[reportGeneralTypeIssues]  (statsbombpy sb is untyped; fmt="dict" => a dict)
    ...
    for match_id, m in list(matches.items())[:6]:  # a handful is enough to fit a serviceable xt + completion model
        try:
            ...
            events = list(sb.events(match_id=int(match_id), fmt="dict").values())  # type: ignore[reportCallIssue]  (statsbombpy sb is untyped; sb.events is a function)
