...
REVIVAL extension: the validator's corpus is repointed at the FULL PUBLIC open-data corpus
(``all_open_competitions()``) with a ``--competitions-json`` override -- so this also pins that
``main`` iterates the broad manifest (monkeypatched stub + tiny fixture), NOT a single competition,
and re-asserts that ``decide_promotion`` reads ONLY the locked constants (the pre-registration gate).
...
def test_validate_driver_is_enrolled_in_the_input_contract_registry():
    """validate_territory_counterfactual declares an input_contract(), so it must be in _DECLARING --
    otherwise the staleness detector never checks its committed artifacts (ADR-056)."""
    from tests.scripts.test_input_contracts import _DECLARING

    assert "validate_territory_counterfactual" in _DECLARING
...
def test_decide_promotion_gate_reads_ONLY_the_locked_constants_no_other_module_global():
    """REVIVAL extension: the gate's every threshold traces to a LOCKED pre-registered constant, and it
    references NO other module-level global (a runtime-derived value must never leak into the gate --
    the TF-19 'gate cannot be moved after seeing the numbers' idiom). The four floors are the only
    module globals decide_promotion is permitted to read; anything else it names is a parameter, a
    local, or a builtin (float/bool)."""
    import scripts.validate_territory_counterfactual as v
    ...
    # Names bound INSIDE the function (params + locals) are not module reads.
    params = {a.arg for a in fn.args.args} | {a.arg for a in fn.args.kwonlyargs}
    assigned = {t.id for n in ast.walk(fn) if isinstance(n, ast.Assign) for t in n.targets if isinstance(t, ast.Name)}
    local = params | assigned
    # Every LOADed Name that is also a module-level attribute must be one of the locked constants.
    module_globals = {k for k in vars(v) if not k.startswith("__")}
    read = {n.id for n in ast.walk(fn) if isinstance(n, ast.Name) and isinstance(n.ctx, ast.Load) and n.id not in local}
    leaked = (read & module_globals) - locked - {"brier_skill_score"}  # the pure BSS helper is allowed
    assert not leaked, (
        f"decide_promotion reads module globals other than the locked constants: {sorted(leaked)} -- a "
        "runtime value must not leak into the pre-registered gate"
    )
    assert locked <= read, "decide_promotion must reference every locked threshold constant by name"
...
# --------------------------------------------------- corpus repoint: the FULL open-data manifest (offline)
# REVIVAL delta: main() iterates all_open_competitions() (the broad public manifest), NOT a single
# competition. Monkeypatched to a 2-tuple stub + a tiny per-competition fixture; the network + the
# battery are stubbed out so this is an OFFLINE smoke of the corpus wiring, not the owner run.


def _fake_match(provider, match_id):
    """A minimal 5-tuple (provider, match_id, actions, frames, home_team_id)."""
...
def test_main_iterates_all_open_competitions_not_a_single_competition(monkeypatch, tmp_path, capsys):
    ...
    to a single hard-coded competition would pull only one tuple's matches and this would see the wrong
    ids / the wrong competition count.
    """
    import scripts._driver as driver_mod
    import scripts._sb_open_data as sbmod
    import scripts.validate_territory_counterfactual as v

    # The FULL manifest stub: TWO distinct (competition, season) tuples.
    ...
    # Capture WHAT main iterates into for_each, then short-circuit the corpus write (return an empty
    # generation) so the battery never runs -- this test pins the CORPUS WIRING, not the numerics.
    seen_ids: list[str] = []
    seen_tokens: dict = {}
    ...
    # No shards written -> run_battery is never reached; main raises SystemExit("no match shards").
    with pytest.raises(SystemExit):
        monkeypatch.setattr("sys.argv", ["prog", "--out", str(tmp_path)])
        v.main()

    # BOTH competitions were walked (the repoint), not just one.
    assert seen_ids == [("statsbomb", "m-43-106"), ("statsbomb", "m-11-90")], seen_ids
    # The broad-corpus marker is in the shard-generation token (the 4.77.1 stale-shard rule).
    assert seen_tokens.get("source") == "open-data-all"


def test_main_narrows_with_competitions_json_and_keys_the_token_disjointly(monkeypatch, tmp_path):
    """--competitions-json narrows to a chosen list AND keys the token on it, so a narrowed generation
    disjoins from the full-manifest one (the corpus marker must differ)."""
    import json
    ...
    # all_open_competitions must NOT be consulted when --competitions-json is given.
    def _must_not_call():
    ...
    with pytest.raises(SystemExit):  # empty generation -> "no match shards"
        monkeypatch.setattr("sys.argv", ["prog", "--out", str(tmp_path), "--competitions-json", str(comps_file)])
        v.main()

    assert seen_ids == [("statsbomb", "m-7-27")]
    # The narrowed token is NOT the full-manifest marker -> the two generations disjoin.
    assert seen_tokens.get("source") != "open-data-all"
    assert seen_tokens.get("source") == {"open-data-narrowed": [(7, 27)]}
