...
- **Tracking-frame coords are float32 STORAGE / float64 COMPUTE; `team_id` is `category`, `player_id` is not** — coords store float32; every kernel upcasts the slice to float64 at its boundary (drift = deterministic storage-rounding, numba/ADR-076 bit-identity intact; `test_frame_coord_upcast_gate`). `team_id`→`category` (static/low-card); `player_id` stays Int64/object (dynamic post-build, ADR-103 rule); `id_compat._decat` unwraps it (value-neutral; ADR-019 category-axis gate). Frames only — SPADL actions stay float64. (ADR-106; `docs/context/tracking-features.md`)
