...
- **SPADL** -- Soccer Player Action Description Language: a unified schema for
  ...
  Opta, Sportec / IDSSE Bundesliga, Metrica Sports, Gradient Sports, and SkillCorner
  — plus a kloppy gateway for raw-provider-data consumers (StatsBomb, Sportec, Metrica)
- **Tracking** -- 20-column long-form per-frame schema parallel to SPADL, with
  native adapters for Sportec / IDSSE Bundesliga, Gradient Sports, SkillCorner, and
  Metrica plus a kloppy gateway (Metrica + SkillCorner) for raw-TrackingDataset
  consumers. Key capabilities: action-to-frame linkage,
