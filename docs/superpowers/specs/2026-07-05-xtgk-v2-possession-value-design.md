...
- Fail-loud on missing pressure (tracking gap); never default a level.
...
- **Deep-zone pressure coverage (G8 — verify BEFORE trusting the gate):** the aggregate "pressure populated" figure is not sufficient. With fail-loud-on-missing-pressure (§5), any deep-zone action lacking a linked-frame pressure value is dropped — potentially decimating the exact cells the gate reads. Confirm pressure coverage **on deep-zone build-up + goal-kick actions specifically** (not in aggregate) per cohort before relying on the gate; report the drop rate.
- **Metrica excluded** (GK actions unscored in current gold; stale-data deferral).
- **Reward input dependency (the one true external need) — B2/Q3, OWNER DECISION:** an `xg_column` with a **calibrated per-shot xG on ALL shots (not just goals)** in both cohorts. silly-kicks ships no xG model; V is fittable/validatable **only where per-shot xG exists** (= these two cohorts), which is acceptable for a validation-gated sub-project — stated plainly. **Design commits to the injected-`xg_column` pattern now; the exact source is an owner-confirmed open item** with a hard gate before fitting: *verify the column is populated on all shots and is calibrated* (an uncalibrated reward poisons V exactly as an uncalibrated ρ would — Eyestone §2.2 applies to the reward too). Candidate sources: a gold `fct_action_context` xG column, or the HF `luxury-lakehouse/xg-model` (confirm pre-shot, not post-shot).
...
2. **Canonical `xg_column` source** (§6/B2) — owner decision; gate on all-shots coverage + calibration before fitting. (Q3.)
