...
## Appendix A — RQ3 stays out of scope (2026-07-28: reason strengthened, decision unchanged)

Section 1 scoped out the paper's RQ3 (defensive positioning optimisation) as "a coaching tool, not a
per-frame VAEP feature". That reason still stands on its own. A re-read of the paper's RQ3 evidence
adds a second, independent reason: **the headline result does not support what it is cited for.**

**No new ADR.** An ADR records a change of decision. This is new evidence for a decision already
recorded, so it belongs in the design doc that recorded it.

### The circularity

RQ3's headline is that the optimiser's suggested position lands inside the Cone Corridor in **789 of
822** cases. That number cannot discriminate a good suggestion from a bad one:

1. Candidate positions are generated by an optimiser that is **constrained toward the Cone
   Corridor** in the first place;
2. success is then scored as *"is the suggested position in the cone?"*.

The metric and the objective are the same construct, so 789/822 measures **optimiser convergence**,
not defensive improvement. A degenerate optimiser that always returned the corridor centroid would
score 822/822.

### What the non-circular number actually says

The paper's own threat-based result — the one that measures the thing RQ3 claims — is that the
suggested position **reduces threat in ~75% of 63,037 snapshots**. Read plainly, that means the
optimiser makes things **worse about a quarter of the time**, and it is reported as a **sign only**:

| Missing | Why it matters here |
|---|---|
| Magnitude | A 75% win rate with tiny wins and large losses is net-negative. Unknown. |
| Confidence interval | 63,037 snapshots are not independent — they are frames within possessions within matches. The effective n is far smaller than 63,037. |
| Placebo / null | No comparison against a random or naive repositioning, so 75% has no reference point. |

The corridor result is additionally scored using the paper's **worst** model at **7.3% recall**.

### Consequence for this library

None to the shipped code — RQ3 was never implemented. The consequence is for **future work**: if
per-defender positioning suggestions are ever revisited, this evidence is the baseline to beat, and
the bar is a **signed, magnitude-reporting, placebo-controlled** comparison rather than a corridor
membership rate. That bar is also why `blocking_score`'s clamped non-negativity is recorded as a
KNOWN LIMITATION in the feature glossary rather than a property: our metric cannot say a defender
made things worse, so it cannot participate in that comparison as-is.
