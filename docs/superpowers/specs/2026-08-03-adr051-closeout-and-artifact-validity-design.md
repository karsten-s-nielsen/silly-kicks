...
| K10 | **The causal placebo band is QUANTIZED at `1/n_treated`, and its digits overstate its resolution.** `estimate_att` is 1:1 nearest-neighbour matching, so every ablation shift and every placebo draw is an exact multiple of `1/n_treated`. Measured on the shot arm (`n_treated=2143`, spacing **0.00046664**): the 4.68.0 shift `0.006999533364442367` and the 4.74.0 band are the SAME lattice point **k=15**, and the 4.74.0 shift is **k=13** -- so a 16-digit collision between two different runs' quantities is expected, not evidence of aliasing. Only ~**21** band values are achievable below the 0.01 `GK_ABLATION_MIN_SHIFT` floor. | ✅ measured | **Not a defect -- a resolution limit that must travel with the number.** Recorded because it cost a full investigation to rule out three plausible bugs (a value carried into `band_p95`; a wrong 4.68.0 shift; `_cluster_reassign` returning identity so the null contains its own alternative). All three were REJECTED: the artifact reproduces exactly from its own shards, and the 5 draws equalling the real shift to `0.000e+00` are lattice coincidences, not identity reassignments -- which is precisely the reading a shifts-array dump alone would have gotten wrong. **Consequences to carry:** do not quote these bands to more than ~4 significant figures; a band cannot resolve differences finer than `1/n_treated`; and an arm with fewer treated units gets a coarser band, so the floor does more of the deciding as `n_treated` falls. Verdict robustness here is comfortable -- the 0.01 floor sits at 21.43/2143, between lattice points, so the band would need to move six steps to matter. |
