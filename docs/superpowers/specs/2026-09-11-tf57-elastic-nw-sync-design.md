...
| SO-3 | **Per-commit approval** of the exact diff. | commit-1 (feature) ☑ **approved + committed** (`e611aa1`, 2026-09-14; pushed, PR #239, CI green); commit-2 (provenance/validation) ☐ pending owner go-ahead. |
...
- **Scoring provenance (owner-approved 2026-09-14; IMPL-07).** The `_score` weights (`w_ba/w_pbd/w_kd/w_dyn` = 0.6/1.6/0.8/1.0 + a directional depart/arrive slope) are **OpenEvolve-tuned** on the 3-match CC-BY corpus — a **data-fit, NOT an MPL-source read** (the §16.4 clean-room boundary held; only the paper + our own code were read). This **RELAXES the §2 non-goal** ("no learned/per-provider-tuned scoring; future tuning is a separate ADR-009 cycle") **by explicit owner decision, recorded here** rather than deferred. Run config: OpenEvolve, `random_seed=42`, 120 iterations, population 40 / 3 islands, sonnet-5 (w 0.8) + opus-4.8 (w 0.2), fitness = score-peak (primary) + min-fold-W2 + coverage guards. The run **provenance artifact** (objective/folds/seeds/config) + a **held-out** (leave-one-match-out) validation landed in the paired **provenance commit** (`docs/research/tf57_elastic_scoring_tuning/`), run clean-tree on the DGX against `e611aa1`: **mean held-out W2 0.860 ≈ in-sample 0.862** (the LOO-refit weights sit in the shipped neighborhood; the shipped vector ranks top-3–8 of 27 per fit-pair), so the ship bar is **not** in-sample-inflated. The committed-oracle floor is measured on a fit match (**in-sample**) — the regression guard, not the acceptance number.
