...
- **Corpus feasibility (AMENDED 2026-09-22 — owner ruling: the FULL owner-tier corpus, never a subset).** At ~1–2 s/frame × 1 fps domain sampling × a few hundred domain-frames/match → **order 10–30 min/match**. Commit-2 runs the **FULL velocity-bearing owner-tier tracking corpus** — every match the pining loader lists for `skillcorner,idsse,gradientsports` (NOT the event-only open-data corpus) — sharded via ADR-052 `for_each` across parallel `--match-ids-json` workers, so wall-clock = total cost ÷ parallelism. The corpus size IS the statistical power of the ship/demote decision, so it is never reduced to make a run feasible; an infeasible run is fixed by ENGINEERING, not by shrinking the sample (next bullet). *Superseded text: "a BOUNDED velocity-bearing tracking corpus (order dozens of matches) … chosen for pooled-probe POWER, not maximal coverage."*
- **Corpus-scale via the shipped load-seam (4.124.0; ADR-052 D13/D14/D15 + ADR-102).** The first full-corpus attempt hit two ENGINEERING blockers, neither a corpus-size problem: the within-corpus xT fit ran `list(load_matches(..., tracking_limit=0))`, which downloaded + parsed every match's tracking (`tracking_limit` caps frames only after the parse) and held every match's actions at once (OOM), although xT is event-only; and the streaming score loop re-loaded on resume. Both are now fixed by ADOPTING the shipped seam rather than bespoke driver code (the amendment's originally-separate "driver-scale-fix commit" is SUBSUMED by 4.124.0, so the release stays 2-commit, §11): the fit is a RESUMABLE EVENTS-ONLY count pass (`_xt_corpus.xt_count_pass` + `fit_xt_from_count_pass` — additive per-match zone counts, byte-identical to a pooled `ExpectedThreat().fit`, never a frame loaded); events-only SkillCorner admission is policy at the edge (`events_only_loader`, `--allow-unmeasured` admits + records); scoring is a resume-before-load `pining_source`/`for_each(refs, load=)` pass (each worker loads only its `--match-ids-json` slice's frames). Fit-once-shared across parallel workers via `--fit-only --xt-path XT.json` (SK-XT-SER JSON, ADR-100) → workers `--xt-path`-load the one coherent surface.
...
- **Feasibility gate (surface, don't silently ship):** if the commit-1 benchmark shows the frozen defaults make the full corpus infeasible even under parallel sharding, the coarse-grid-search lever lands as a **commit-1 addition BEFORE the commit-2 run** — never a deferred follow-up that ships an un-runnable battery.
...
- **Commit-1** (clean tree, human-approval gate): all code + protocols + solver + compute + probes + docs (this spec + the plan + the ADR) + C4 + glossary + `metric_contracts` + NOTICE + fixture tests. NO bundled weights. **The corpus-scale driver fix folds into commit-1 (§8b): it ADOPTS the 4.124.0 shipped load-seam (events-only `xt_count_pass` fit + `pining_source`/`for_each` score), so the amendment's originally-separate "driver-scale-fix commit" is SUBSUMED and the release stays 2-commit.** (The branch is based on current `main` = 4.124.0.)
