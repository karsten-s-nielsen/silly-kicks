...
- [ ] **Step 3:** version to all 5 places at commit-prep only (`pyproject.toml`, `__init__.py`, `uv.lock`, `CHANGELOG` heading, `TODO` Release) — **4.91.0 / PR-S162 / ADR-066** (the other session shipped 4.90.0/PR-S160 AND 4.90.1/PR-S161 first, so both are taken; ADR-066 is free), re-verified against `origin/main` first.
...
- **4.89.0 merge:** ADR-065/PR-S159 taken → renumbered to ADR-066, then 4.90.0/PR-S160; next-touch/failure-mode logic builds on the chronological-`action_id` guarantee + `_sort_actions_chronological_or_action_id` (T8); GS byte-identical (no retrain) but now needs a `start_time` input column (loader supplies).
- **4.90.x merge (second collision):** the other session then shipped 4.90.0/PR-S160 (SB360 velocity hardening, ADR-054+063 amendments) + 4.90.1/PR-S161 (sportec set-piece) → this cycle renumbered again to **4.91.0/PR-S162**; ADR-066 stayed free. Merged `origin/main`: only doc/version conflicts, no code-file overlap; main's ADR-063 velocity change is byte-identical on velocity-bearing GS, so the de-leak / cover-shadow / pitch-control artifacts are unaffected.
