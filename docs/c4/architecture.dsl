workspace "silly-kicks" "Football action classification (SPADL) and valuation (VAEP) library" {
    ...
    model {
        ...
        sillyKicks = softwareSystem "silly-kicks" "Classifies football actions into SPADL representation and values them via VAEP" {
            ...
            tracking = container "silly_kicks.tracking" "Per-frame tracking: schema, provider adapters (native+kloppy), event-frame linkage, spatial/GKDV models (pitch control, OBSO, DAS, ghost-GK, xS/xCross/xT-GK) with fail-closed chirality load() enforcement, 29 action-coupled aggregators. ADR-039, ADR-040." "Python" "Library"
