workspace "silly-kicks" "Football action classification (SPADL) and valuation (VAEP) library" {
    ...
    model {
        ...
        sillyKicks = softwareSystem "silly-kicks" "Classifies football actions into SPADL representation and values them via VAEP" {
            ...
            tracking = container "silly_kicks.tracking" "Per-frame tracking data: 20-column long-form schema, 4 provider adapters, event-frame linkage, preprocessing (smoothing, interpolation, velocities), and action-coupled features (pressure, defensive line, ball carrier, GK position)." "Python" "Library"
