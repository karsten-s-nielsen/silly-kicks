workspace "silly-kicks" "Football action classification (SPADL) and valuation (VAEP) library" {
    ...
    model {
        ...
        sillyKicks = softwareSystem "silly-kicks" "Classifies football actions into SPADL representation and values them via VAEP" {
            ...
            tracking = container "silly_kicks.tracking" "Per-frame tracking data: schema, provider adapters, event-frame linkage, preprocessing, pitch control, OBSO, DAS, space creation, structural-pass primitives (LBS/SGM/SDI), ghost-GK positioning (selectable scipy/vectorized/cpu-numba/fft/fft-cic KDE backends; serves the exact pickle-free boosted-mean estimator), shot-occurrence (xS), cross-attempt (xCross), Eyestone xT-GK (GK-distribution value; RAV completion via a fitted GkCompletionModel with a per-type base-rate serve gate + scoped goal-kick coordinate derivation), post-shot goalmouth crossing geometry (TF-48: pure trajectory-fit engine with sample-and-hold collapse, contact anchoring/existence, z-aware flight and an extrapolation-leverage cap; lakehouse-PSxG feed, holdout-validated), SkillCorner + Metrica bronze-to-frame builders (tracking.skillcorner / tracking.metrica) that single-source the per-provider coordinate rescale + period-relative clock + id-namespacing + GK derivation + LTR orientation the lakehouse previously duplicated, promoting the lakehouse ADR-053 geometric frame-LTR net to the library (orient_frames_to_ltr_by_geometry) and recovering SkillCorner ball_z (ADR-034), and 28 action-coupled aggregators. Enforced consumer contracts: period-relative time base (validate_time_base, ADR-017), dtype-safe id comparison/merge at the feature seams (_id_compat + validate_id_dtypes, ADR-019), position-based frame_id resolution in frame-aware xfns (resolve_frame_ids_by_position, ADR-020), per-action SPADL-LTR re-projection of emitted geometry at the feature seams (_action_orientation, ADR-028), single-sourced frame-LTR orientation for consumer-built frames (orient_frames_to_ltr, ADR-029), the kloppy tracking gateway pinning the canonical SPADL coordinate system shared with the event gateway (was y-inverted; ADR-031), and input-purity across the whole add_* enrichment family (helpers never mutate caller inputs and return a new object; auto-enumerating CI gate, ADR-033)." "Python" "Library"
