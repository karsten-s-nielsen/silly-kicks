workspace "silly-kicks" "Football action classification (SPADL) and valuation (VAEP) library" {
    ...
    model {
        ...
        sillyKicks = softwareSystem "silly-kicks" "Classifies football actions into SPADL representation and values them via VAEP" {
            ...
            expected_passing = container "silly_kicks.expected_passing" "Event-only Expected-Passing model (PassCompletionModel): P(pass completes | origin->target geometry); pickle-free JSON+SHA256 with chirality + feature-contract fail-closed load. ADR-089." "Python" "Library"
        ...
        territory -> expected_passing "Injects a fitted pass-completion model for the counterfactual prevented-valuation via" "predict_completion()"
        expected_passing -> spadl "Reads SPADL action-type / result ids from" "Python import"
