workspace "silly-kicks" "Football action classification (SPADL) and valuation (VAEP) library" {
    ...
    model {
        ...
        sillyKicks = softwareSystem "silly-kicks" "Classifies football actions into SPADL representation and values them via VAEP" {
            ...
            tracking = container "silly_kicks.tracking" "Tracking namespace (ADR-004): 20-column long-form per-frame schema (is_goalkeeper_source added PR-S26), native Sportec + PFF adapters, kloppy gateway for Metrica + SkillCorner, link_actions_to_frames + slice_around_event linkage primitives. PR-S26 (ADR-007, 3.3.0) ships is_goalkeeper hardening: _gk_identification.derive_goalkeepers B+ filtered positional algorithm (always-run with agreement-based source resolution) fixes Metrica/SkillCorner 21-50% to 100% GK detection; is_goalkeeper_source column (native/derived provenance); TrackingConversionReport extended with n_teams_gk_derived + derived_gk_picks audit fields. PR-S20 (ADR-005) shipped the first tracking-aware features: nearest_defender_distance, actor_speed, receiver_zone_density, defenders_in_triangle_to_goal + add_action_context aggregator + tracking_default_xfns. PR-S21 added pre-shot GK position: pre_shot_gk_x / _y / _distance_to_goal / _distance_to_shot + add_pre_shot_gk_position aggregator + pre_shot_gk_default_xfns. PR-S22 (ADR-006, 3.0.0) added output_convention='absolute_frame'|'ltr' kwarg to all 3 adapters; default behaviour preserved (DeprecationWarning recommends explicit). PR-S24 (3.1.0) ships TF-6 sync_score (per-action tracking/events sync-quality, 3 aggregations) + TF-12 pre_shot_gk_angle_to_shot_trajectory / _off_goal_line (signed angles) + add_pre_shot_gk_angle aggregator + pre_shot_gk_angle_default_xfns / pre_shot_gk_full_default_xfns. silly_kicks.tracking.preprocess submodule ships TF-8 smooth_frames (Savitzky-Golay / EMA -- additive x_smoothed/y_smoothed/_preprocessed_with cols) + derive_velocities (vx/vy/speed via SG-derivative) + TF-9 interpolate_frames (linear NaN gap-fill up to max_gap_seconds; cubic deferred to TF-9-cubic) + shared PreprocessConfig dataclass with flag-based is_default() + per-provider defaults from a codegen-from-JSON pipeline (probe_preprocess_baseline.py -> preprocess_baseline.json -> regenerate_provider_defaults.py -> _provider_defaults_generated.py). Tracking converters take optional preprocess: PreprocessConfig | None = None kwarg with auto-promotion via _resolve.resolve_preprocess (UserWarning + force_universal fallback for unsupported providers). PR-S25 (3.2.0) ships TF-3 actor pre-window features: actor_arc_length_pre_window + actor_displacement_pre_window + add_actor_pre_window + actor_pre_window_default_xfns (default xfn list ships arc-length only). NEW silly_kicks.tracking.pressure submodule ships TF-2 multi-flavor pressure_on_actor with three published methodologies (andrienko_oval default = Andrienko 2017; link_zones = Link 2016; bekkers_pi = Bekkers 2024 BSD-3-Clause UnravelSports port) + per-method frozen parameter dataclasses (AndrienkoParams / LinkParams / BekkersParams) + Method Literal + validate_params_for_method. add_pressure_on_actor umbrella; pressure_default_xfns (default ships single Andrienko flavor per ADR-005 multi-flavor xfn column-naming convention; column pattern pressure_on_actor__<method>). Schema-agnostic kernels in _kernels shared with atomic SPADL surface. PR-S27 ships TF-13 defending_gk_from_frames (frame-based GK resolution fallback via is_goalkeeper lookup on opposing team in linked frame) in _gk_resolve.py + TF-14 compute_defensive_line (per-frame 6-column back-line geometry: defensive_line_x, back_line_high_x, compactness_x, lateral_width, max_lateral_gap, back_n_count; adaptive N via x-gap clustering) in _defensive_line.py + 6 action-coupled per-Series features + add_defensive_line aggregator + defensive_line_xfns factory (batch kernel pattern via _row_idx positional key for gamestates compatibility)" "Python" "Library"
