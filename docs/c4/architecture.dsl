workspace "silly-kicks" "Football action classification (SPADL) and valuation (VAEP) library" {
    ...
    model {
        ...
        sillyKicks = softwareSystem "silly-kicks" "Classifies football actions into SPADL representation and values them via VAEP" {
            ...
            match_outcome = container "silly_kicks.match_outcome" "TF-53 event-only per-(game,team) win/draw/loss + xPoints from injected per-shot xG: exact Poisson-binomial goal PMF; opt-in possession-collapse + Dixon-Coles dependence corrections." "Python" "Library"
            win_probability = container "silly_kicks.win_probability" "TF-63 in-game win probability + xImpact: interval-hazard GLM feeds a self-contained Markov chain on score_diff; goal_leverage weights VAEP_adjusted into VAEP.rate_ximpact. Event-only. ADR-101." "Python" "Library"
        ...
        analyst -> win_probability "Computes in-game win probability + goal leverage via" "compute_win_probability() / goal_leverage()"
        win_probability -> spadl "Reads SPADL actions + canonical id helpers from" "Python import"
        vaep -> win_probability "Weights VAEP_adjusted by goal leverage into xImpact (VAEP.rate_ximpact) via" "lazy import"
        // --- Relationships: TF-56 prescriptive defensive-positioning optimiser ---
        analyst -> positioning "Optimises the best reachable defensive shape + scores positioning_gap (INJECTED fitted xt) via" "optimise_positions() / compute_positioning_gap()"
        positioning -> tracking "Reads xT-weighted pitch-control threat, kinematic TTI, goal map, ball-carrier from" "compute_threat_pc / compute_tti / resolve_defended_goals / infer_ball_carrier (PUBLIC seams; never reverse, allowlist-gated)"
        positioning -> spadl "Reads pitch geometry (field dims) + canonical id helpers from" "Python import"
        ...
        // --- Relationships: TF-53 event-only match outcome / win-probability / xPoints ---
        analyst -> match_outcome "Computes per-(game, team) win/draw/loss + xPoints from injected per-shot xG via" "compute_match_outcome()"
        match_outcome -> spadl "Reads SPADL actions, add_possessions + canonical id helpers from" "Python import"
