workspace "silly-kicks" "Football action classification (SPADL) and valuation (VAEP) library" {
    ...
    model {
        ...
        sillyKicks = softwareSystem "silly-kicks" "Classifies football actions into SPADL representation and values them via VAEP" {
            ...
            win_probability = container "silly_kicks.win_probability" "TF-63 in-game win probability + xImpact: interval-hazard GLM feeds a self-contained Markov chain on score_diff; goal_leverage weights VAEP_adjusted into VAEP.rate_ximpact. Event-only. ADR-100." "Python" "Library"
            ...
            glossary = container "silly_kicks.feature_glossary + reporting" "Machine-readable glossary of all 456 derived feature columns (CI-gated, NOTICE-linked, inspection-enumerated) + describe_level direction-aware z-bucket reporting helper. ADR-048." "Python" "Library"
