workspace "silly-kicks" "Football action classification (SPADL) and valuation (VAEP) library" {
    ...
    model {
        ...
        sillyKicks = softwareSystem "silly-kicks" "Classifies football actions into SPADL representation and values them via VAEP" {
            ...
            tracking = container "silly_kicks.tracking" "Per-frame tracking data: schema, provider adapters, event-frame linkage, preprocessing, pitch control, OBSO, DAS, space creation, ghost-GK positioning (selectable scipy/vectorized/cpu-numba/fft/fft-cic KDE backends; serves the exact pickle-free boosted-mean estimator), shot-occurrence (xS), cross-attempt (xCross), and 23 action-coupled aggregators. Enforced consumer contracts: period-relative time base (validate_time_base, ADR-017) and dtype-safe id comparison/merge at the feature seams (_id_compat + validate_id_dtypes, ADR-019)." "Python" "Library"
