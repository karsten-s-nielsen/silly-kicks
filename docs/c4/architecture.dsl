workspace "silly-kicks" "Football action classification (SPADL) and valuation (VAEP) library" {
    ...
    model {
        ...
        sillyKicks = softwareSystem "silly-kicks" "Classifies football actions into SPADL representation and values them via VAEP" {
            ...
            tracking = container "silly_kicks.tracking" "Per-frame tracking: schema, adapters, linkage, spatial/GKDV models (pitch control, OBSO, DAS, ghost-GK, xS/xCross/xT-GK, defensive credit), 32 action-coupled aggregators, feature-contract guard." "Python" "Library"
            ...
            calibration = container "silly_kicks.calibration + scripts/" "Optuna calibration harness (pure objectives/CV/gates + frozen exogenous xT artifact) + scripts/ CLI, pining/Databricks loaders and trainers. Recommends tuned defaults; never changes library constants." "Python (optional [calibration] extra)" "Library"
