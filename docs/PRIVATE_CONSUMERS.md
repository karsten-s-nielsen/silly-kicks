...
| `add_space_creation` output `space_opponent_source` | **NEW additive column** (`resolved` / `unresolved_one_team`) alongside `space_denied_m2_opponent` (ADR-054 amendment, next release). The primary `space_created_m2` / `space_denied_m2_opponent` columns are byte-identical | Any consumer that materializes space_creation (action-context schema / DDL / mart contract) | Opponent-perspective now softens a one-team SB360 FOV crop to a per-row NaN opponent value with a provenance token instead of aborting the batch | **Non-breaking** — add the additive column (unknown columns are ignored); note `space_denied_m2_opponent` is NaN exactly when `space_opponent_source == "unresolved_one_team"` |
| `add_xshot_occurrence` / `xshot_occurrence_xfns` output on SB360 | Velocity-less SB360 freeze-frames flip from a **fabricated** probability to honest NaN (ADR-054 amendment) — was live on the `pre_shot_gk_full_default_xfns` GK-union bundle | Any consumer building `pre_shot_gk_full_default_xfns` (or scoring xShot) on SB360 | `speed` is a trained feature; XGBoost missing-value routing produced an ungrounded value on velocity-less frames (the ADR-053 shape) | Re-check SB360 xShot null-handling; the old values were meaningless (a model trained on them was trained on a bug). Full-tracking is byte-identical |
| `pressure_on_actor__bekkers_pi` on SB360 | Velocity-less SB360 frames flip from a `ValueError` ("run `derive_velocities()`", impossible on a freeze-frame) to honest NaN (ADR-063 amendment, Tier-3) | Any consumer calling `add_pressure_on_actor(methods=(…,"bekkers_pi"))` on SB360 (opt-in; not the default battery), incl. the marts `pressure_on_actor__bekkers_pi` that feeds `GkRetention.release_pressure` | bekkers_pi's active-pressing `speed_threshold` filter makes its zero-velocity form artifact-dependent, not a smooth limit | Marts `pressure_on_actor__bekkers_pi` is now honest-NaN on SB360 rows; retention's `release_pressure` input is honest (mean-imputed downstream). `andrienko_oval` / `link_zones` and full-tracking unchanged |
| `add_xshot_occurrence` / `add_xcross_attempt` / `add_ghost_gk` on SB360 + new `*_variant` columns | Velocity-less SB360 frames flip from honest-NaN to a real **position-only** value ONCE the position_only variants are bundled (ADR-067, velocity-keyed auto-select); a NEW closed-vocab provenance column `xshot_occurrence_variant` / `xcross_attempt_variant` / `ghost_gk_variant` (`{default, position_only, custom}`) is added on each `add_*`; gkdv's ghost arm begins producing on SB360 | Any consumer materializing these on SB360, building `pre_shot_gk_full_default_xfns`, or consuming gkdv on SB360 | The position-only variants are the intended unlock (make SB360 produce a value, not NaN); the provenance column makes the NaN→value change auditable per action | **Velocity-bearing frames are byte-identical.** The numeric `*_xfns` / VAEP path has NO per-cell provenance — key traceability off the frame-level velocity marker or the match provider, not the cell value. Add the additive `*_variant` columns (unknown columns are ignored). Until the variants are bundled (Phase B), SB360 stays NaN with `*_variant == "position_only"` |
| `gkdv.delta_das` / `gkdv.delta_threat_suppression` on SB360 | Once the position-only ghost is bundled (ADR-067) gkdv's counterfactual arms become **reachable** on velocity-less SB360 frames: `delta_das` now degrades to honest-**NaN** (the ADR-043 consumer-degrade — `DasUnscoreableError` caught in `delta_das`) instead of **RAISING**, and `delta_threat_suppression` produces a positional-only value (pitch control's zero-velocity model, ADR-063) where the frame used to be dropped | Any consumer running gkdv's counterfactual arms over an SB360 (velocity-less) frame set | Reverts an interim "gkdv declines velocity-less wholesale" approach: DAS structurally needs velocity so it degrades honestly rather than crashing, while pitch-control threat has a valid positional model | A gkdv SB360 pass **no longer crashes on `delta_das`** — expect honest-NaN there; `delta_threat_suppression` now carries a positional-only value on SB360. **Velocity-bearing frames are byte-identical.** gkdv ships no `add_*` aggregator, so the arms (not a column) are the surface — key handling off the frame-level velocity marker |
...
| `tracking/_provider_visibility.py` | `validate_provider`, `assert_detection_aware_visibility`, `_DETECTION_AWARE_PROVIDERS`, `_detection_discarded_message` | `scripts/train_ghost_gk.py` (consume-time pre-flight), `scripts/materialize_tc3_frames.py` (build-time guard), `tests/scripts/test_trainer_cache_and_providers.py`, `tests/tracking/test_detection_aware_visibility.py` | The provider visibility taxonomy + the detection-aware visibility-usability rule (ADR-069: detection-aware visibility guardrails), moved OUT of `_ghost_gk` under the 4.53.0 `_id_compat` clean-break precedent so a general tc3 builder does not import from a ghost-private module. `_ghost_gk` itself references these via a module alias (`_pv.*`), never re-exporting them. Real `import`s -> a rename fails **loudly** at collection. **NB:** the `_ghost_gk.py` module-PATH drift-guard pin above is unaffected (that module stays present; only these symbols moved to a sibling). | Promote to `tracking.__all__` only if a cross-package consumer appears (YAGNI; the id_compat path). |
| `_frame_index.py` | `group_rows`, `RowGroups` (O(1) dtype-safe row-group lookup) | `causal/`, `tracking/`, `spadl/`, `restdefense/` (TF-60 `count_goalside_by_sample` + the `compute_rest_defense` per-sample loop, ADR-080), plus `scripts/_loader_databricks.py` (client-side per-match split) (rescan-in-loop remediation, ADR-068, 2026-08-24) | The recurring `df[df[k]==v]`-in-a-loop O(n²) fix routes through one seam rather than nine inline `groupby`s — one home for the ADR-019 key-canonicalisation collision guard + one structural-guard surface. Private leaf util (`_geometry`/`_polygon` position), NOT `id_compat` (which is public because ADR-019 makes it *mandatory*). Real `import`s -> a rename fails **loudly** at collection. | Promote to `silly_kicks.__all__` only if a cross-package/cross-repo consumer appears; until then recording here suffices (YAGNI). |
