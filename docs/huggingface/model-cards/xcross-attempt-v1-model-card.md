...
| `sc_extended_position_only` | 179 owner-tier matches, **velocity features dropped** (15-feature) | **separate repo** `silly-kicks/xcross-attempt-position-only-v1` (HF-only) | Yes, if scoring **velocity-less** frames (StatsBomb-360 freeze frames) with owner-tier access — a stronger position-only model than the bundled `position_only`. Reachable ONLY via `from_variant("sc_extended_position_only")`; asking for `sc_extended` still returns the **faithful** model (ADR-070). |
