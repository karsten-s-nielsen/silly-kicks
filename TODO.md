...
| — | Medium | Atomic-SPADL parity for `add_possessions` + GK analytics suite | The SPADL post-conversion enrichments (`add_possessions` 1.2.0, `add_gk_role` / `add_gk_distribution_metrics` / `add_pre_shot_gk_context` 1.4.0) all operate on the standard SPADL action stream. Atomic-SPADL has a different action-type set (receival, interception, out, etc.), so the role/distribution/lookback algorithms need adaptation. Same disposition as the `preserve_native` Atomic-SPADL parity entry — apply when a concrete consumer asks. |
