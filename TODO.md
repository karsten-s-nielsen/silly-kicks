...
**Last updated**: 2026-05-05. **Current release**: silly-kicks 3.5.0-dev. Per-version history lives in [CHANGELOG.md](CHANGELOG.md).
...
| # | Task | Size | Source | Notes |
|---|------|------|--------|-------|
| TF-4 | `off_ball_runs` — attacking teammate runs in pre-action window + line-break detection | Wicked | Power et al. 2017 (OBSO §3 off-ball-runs + §4 line-break); Spearman 2018; Decroos & Davis 2020 | **Multi-paper synthesis; bundles two sibling action-coupled features.** Off-ball-runs: per-teammate temporal analysis in the pre-action window. Line-break: per-action × line-geometry coupling (attacker behind defending back-line at action destination). Both share Power 2017 OBSO and the same (action × attacker positions × defending-team line) shape; bundling avoids duplicate plumbing. Heaviest of the deferred bench. Line-break was separated from TF-14 in the 2026-05-04 brainstorm — TF-14 is per-frame line geometry (action-independent); line-break is per-action coupled and belongs with off-ball-runs. **References:** Power, P., Ruiz, H., Wei, X., & Lucey, P. (2017), "Not all passes are created equal." KDD '17 (OBSO — Off-Ball Scoring Opportunity; §3 off-ball-runs, §4 line-break). Spearman, W. (2018), "Beyond Expected Goals." MIT Sloan SAC. Decroos, T., & Davis, J. (2020), Player-Vectors blog/extension of VAEP. ~250-350 LOC + scaling considerations. |
